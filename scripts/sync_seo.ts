...
function copyAndReplace(src: string, dest: string, brand: string, url: string) {
    ...
    // Replace "Anexa1" with the target brand
    content = content.replace(/Anexa1/g, brand);
    content = content.replace(/www\.anexa1\.ro/g, url);
    content = content.replace(/Anexa1\.ro/g, url);
...
async function main() {
    for (const proj of PROJECTS) {
        ...
        copyAndReplace(
            path.join(ANEXA1_PATH, 'lib/seo/ro_localitati.json'),
        ...
        // 2. localitati.ts
        copyAndReplace(
            path.join(ANEXA1_PATH, 'lib/localitati.ts'),
        ...
        copyAndReplace(
            path.join(ANEXA1_PATH, 'app/judet/page.tsx'),
        ...
        copyAndReplace(
            path.join(ANEXA1_PATH, 'app/judet/[judetSlug]/page.tsx'),
        ...
        copyAndReplace(
            path.join(ANEXA1_PATH, 'app/judet/[judetSlug]/[localitateSlug]/page.tsx'),
        ...
        copyAndReplace(
            path.join(ANEXA1_PATH, 'app/judet/[judetSlug]/[localitateSlug]/[productSlug]/page.tsx'),
