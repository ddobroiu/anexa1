                ...
                <div className="w-full lg:w-64 flex-shrink-0 space-y-8">
                    ...
                    <div className="relative">
                        <input
                            ...
                            className="w-full pl-4 pr-10 py-3 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none text-sm transition-all"
                            value={searchQuery}
                            ...
                            <div className={`flex-col gap-1 ${isCategoriesOpen ? 'flex' : 'hidden'} lg:flex`}>
                                ...
                                {categories.length > 10 && (
                                    <button
                                        ...
                                        className="text-xs font-bold text-blue-600 hover:text-blue-700 pt-2 pl-2 text-left"
                                    >
                                    ...
                                    {product.categories && product.categories.length > 0 && (
                                        <div className="absolute top-3 left-3 flex flex-wrap gap-1">
                                            {product.categories.filter(c => c !== 'Canvas' && c !== 'Artă Murală' && c !== 'Panouri Sticlă Bucătărie').slice(0, 1).map(c => (
                                                <span key={c} className="bg-blue-600/90 backdrop-blur-sm text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide shadow-sm">
                                                    {c}
                                                </span>
                                            ))}
                                        </div>
                                ...
                                <div className="p-5 flex flex-col flex-grow">
                                    ...
                                    <div className="mt-4 pt-4 border-t border-gray-100">
                                        <Link href={`/${linkBase}/${product.slug}`} className="block">
                                            <Button
                                                className="w-full bg-slate-900 hover:bg-blue-600 text-white rounded-xl h-10 font-bold transition-colors"
                                                size="sm"
                                            >
                                                Vezi Configurare
                                        ...
                                        if (p === 1 || p === totalPages || (p >= currentPage - 2 && p <= currentPage + 2)) {
                                            return (
                                                <Button
                                                    ...
                                                    className={`h-10 w-10 font-bold ${currentPage === p ? 'bg-blue-600 hover:bg-blue-700' : ''}`}
                                                >
                                                    {p}
                                                </Button>
                                            );
