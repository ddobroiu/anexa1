                  ...
                  <AccordionStep stepNumber={2} title="Dimensiune & Tiraj" summary={summaryStep2} isOpen={activeStep === 2} onClick={() => setActiveStep(2)}>
                     <div className="py-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                           {sizes.map((v: any) => (
                              <button key={v.id} onClick={() => setSelectedVariant(v)} className={`px-4 py-2 text-sm font-bold border-2 rounded-lg transition-all ${selectedVariant?.id === v.id ? "border-blue-600 bg-blue-50 text-blue-700" : "border-gray-200 bg-white text-gray-500 hover:border-gray-300"}`}>{v.size}</button>
                           ))}
                           <Link href="/autocolante" className="px-4 py-2 text-sm font-bold border-2 border-dashed border-blue-200 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 flex items-center gap-2"><Ruler size={16} /><span>Dimensiune Custom?</span></Link>
                        </div>
