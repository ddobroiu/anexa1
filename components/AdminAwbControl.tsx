...
export default function AdminAwbControl({ orderId, currentAwb }: { orderId: string; currentAwb?: string | null }) {
  ...
  if (isEditing) {
    return (
      <div className="flex items-center gap-1">
        ...
        <button onClick={saveManualAwb} className="p-1 bg-blue-500/20 text-blue-400 rounded"><Check size={12} /></button>
  ...
  if (awb) {
    return (
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center justify-between bg-blue-500/10 border border-blue-500/20 px-2 py-1 rounded">
          <span className="text-[10px] font-mono font-bold text-blue-400 tracking-wide">{awb}</span>
          <button onClick={() => setIsEditing(true)} className="text-zinc-500 hover:text-white"><Edit2 size={10} /></button>
        </div>
        <div className="flex items-center gap-2">
          <a href={`https://tracking.dpd.ro/?shipmentNumber=${awb}&language=ro`} target="_blank" className="flex items-center gap-1 text-[9px] text-blue-300 hover:text-white hover:underline bg-blue-500/10 py-1 px-2 rounded transition-colors">
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        <Button size="sm" variant="outline" onClick={validateAwb} disabled={loading} className="h-8 text-[10px] bg-blue-50 text-blue-700 hover:bg-blue-100 border-blue-100 flex-1 font-bold">
