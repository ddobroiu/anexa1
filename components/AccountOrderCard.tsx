...
const DELIVERY_LABEL: Record<string, { label: string; tone: string }> = {
    ...
    livrat: { label: "Coletul a fost livrat", tone: "bg-blue-50 text-blue-900" },
...
export default function AccountOrderCard({ order }: { order: Order }) {
    ...
    return (
        <li className="rounded-2xl border border-slate-200 bg-white p-4 transition hover:border-slate-300 hover:shadow-sm sm:p-5">
                ...
                <ol className="mt-4 grid grid-cols-4 gap-1" aria-label="Stadiul comenzii">
                    {STEPS.map((s, i) => (
                        <li key={s} className="min-w-0">
                            <span className={`block h-1.5 rounded-full ${i <= step ? "bg-blue-500" : "bg-slate-200"}`} />
                            <span className={`mt-1.5 flex items-center gap-1 truncate text-[11px] sm:text-xs ${i === step ? "font-semibold text-slate-900" : i < step ? "text-blue-700" : "text-slate-400"}`}>
            ...
            {awbUrl && !canceled && (() => {
                const d = DELIVERY_LABEL[order.delivery?.state ?? ""] ?? { label: "Coletul a fost predat curierului", tone: "bg-blue-50 text-blue-900" };
