                        ...
                        <div className="bg-white rounded-2xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] border border-gray-200 dark:border-slate-800 px-4 mb-8">
                                        ...
                                        <div className="grid grid-cols-3 gap-2">
                                            {FLYER_CONSTANTS.SIZES.map((s: any) => (
                                                <button
                                                    ...
                                                    className={`py-2 text-xs font-bold border-2 rounded-lg transition-all ${sizeKey === s.key ? 'border-slate-950 bg-blue-50 text-blue-500' : 'border-gray-300 bg-white hover:border-gray-400'}`}
                                                >
                                                    {s.label}
                                                </button>
                                            ))}
                                        ...
                                        <div className="grid grid-cols-1 gap-2">
                                            {FLYER_CONSTANTS.PAPER_WEIGHTS.map((p: any) => (
                                                <button
                                                    ...
                                                    className={`py-2 px-3 text-xs font-bold border-2 rounded-lg transition-all ${paperWeightKey === p.key ? 'border-slate-950 bg-blue-50 text-blue-500' : 'border-gray-300 bg-white hover:border-gray-400'}`}
                                                >
                                                    {p.label}
                                                </button>
                                            ))}
                            ...
                            <AccordionStep stepNumber={2} title="Tiraj & Imprimare" summary={summaryStep2} isOpen={activeStep === 2} onClick={() => setActiveStep(2)}>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <NumberInput label="Cantitate (buc)" value={quantity} onChange={setQty} step={10} />
                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Pagini</label>
                                        <div className="grid grid-cols-2 gap-2">
                                            <button onClick={() => setTwoSided(false)} className={`py-3 rounded-lg border-2 text-sm font-bold ${!twoSided ? 'border-slate-950 bg-blue-50 text-blue-500' : 'border-gray-300 bg-white hover:border-gray-400'}`}>Față</button>
                                            <button onClick={() => setTwoSided(true)} className={`py-3 rounded-lg border-2 text-sm font-bold ${twoSided ? 'border-slate-950 bg-blue-50 text-blue-500' : 'border-gray-300 bg-white hover:border-gray-400'}`}>Față-Verso</button>
                                        </div>
                                    </div>
                                </div>
