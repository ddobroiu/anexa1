...
const SelectGroup = ({ label, options, value, onChange }: { label: string, options: { id: string, label: string, price: number }[], value: string, onChange: (val: string) => void }) => (
    <div className="mb-4">
        ...
        <select
            className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-white text-gray-800"
            value={value || "none"}
...
function FonduriTypeSwitch() {
    ...
    return (
        <div className="inline-flex rounded-lg border border-gray-300 bg-white p-1 shadow-sm">
            <Link
                href="/fonduri-pnrr"
                className={`px-3 py-1.5 rounded-md text-xs sm:text-sm font-semibold transition-all inline-flex items-center justify-center ${isPnrr ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 dark:text-gray-400 hover:bg-slate-50 dark:bg-slate-800'}`}
            >
                PNRR
            </Link>
            <Link
                href="/fonduri-regio"
                className={`ml-1 px-3 py-1.5 rounded-md text-xs sm:text-sm font-semibold transition-all inline-flex items-center justify-center ${isRegio ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 dark:text-gray-400 hover:bg-slate-50 dark:bg-slate-800'}`}
            >
                REGIO
            </Link>
            <Link
                href="/fonduri-nationale"
                className={`ml-1 px-3 py-1.5 rounded-md text-xs sm:text-sm font-semibold transition-all inline-flex items-center justify-center ${isNationale ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 dark:text-gray-400 hover:bg-slate-50 dark:bg-slate-800'}`}
            >
                Naționale
            </Link>
        </div>
                            ...
                            <div className="p-2 grid grid-cols-4 gap-2">
                                {GALLERY.map((src, i) => (
                                    <button
                                        ...
                                        className={`relative rounded-lg aspect-square overflow-hidden border-2 transition-all ${activeIndex === i ? "border-blue-600 shadow-md" : "border-transparent opacity-60 hover:opacity-100"}`}
                                    >
                                        <Image 
                                            src={src} 
