...
const TabButton = ({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) => (
    <button
        ...
        className={`px-4 py-2 text-sm font-bold transition-all border-b-2 ${
            active ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-300'
        }`}
    >
        {children}
    </button>
...
const TabButtonSEO = ({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) => (
    <button
        ...
        className={`flex-1 py-3 px-4 text-sm font-bold transition-colors ${
            active ? 'bg-white text-blue-600' : 'text-gray-500 hover:text-gray-700 dark:text-gray-300'
        }`}
    >
        {children}
    </button>
    ...
    return (
        <div className="inline-flex flex-wrap gap-1 rounded-lg border border-gray-300 bg-white p-1 shadow-sm shrink-0">
            <Link
                ...
                className={`px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-semibold transition-all ${isFace ? "bg-blue-600 text-white shadow-md" : "text-gray-600 dark:text-gray-400 hover:bg-slate-50 dark:hover:bg-slate-800"}`}
            >
                O față
            </Link>
            <Link
                href="/configurator/banner-verso"
                className={`px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-semibold transition-all ${isVerso ? "bg-blue-600 text-white shadow-md" : "text-gray-600 dark:text-gray-400 hover:bg-slate-50 dark:hover:bg-slate-800"}`}
            >
                Față-verso
            </Link>
            <Link
                href="/configurator/mesh"
                className={`px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-semibold transition-all ${isMesh ? "bg-blue-600 text-white shadow-md" : "text-gray-600 dark:text-gray-400 hover:bg-slate-50 dark:hover:bg-slate-800"}`}
            >
                Mesh
            </Link>
        </div>
    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-800 min-h-screen pb-20 w-full max-w-full overflow-x-hidden box-border">
            <ProductJsonLd
                ...
                sku={product.id}
                url={`https://www.anexa1.ro/banner-product/${product.slug}`}
            />
                                ...
                                <div className="flex flex-col gap-3 mb-3 sm:mb-4">
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2">
                                        ...
                                        <div className="flex rounded-lg border border-slate-200 bg-slate-50 p-0.5 shrink-0">
                                            <button type="button" onClick={() => setBannerType("single")} className={`flex-1 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${bannerType === "single" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500"}`}>O față (acest model)</button>
                                            <button type="button" onClick={() => setBannerType("double")} className={`flex-1 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${bannerType === "double" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500"}`}>Față-verso PVC</button>
                                        </div>
                                    </div>
                                </div>
                                         ...
                                         <div className="flex flex-wrap gap-1.5 sm:gap-2">
                                             {PREDEFINED_DIMENSIONS.map((dim, idx) => (
                                                 <button 
                                                     ...
                                                     className={`px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg border font-bold text-[10px] sm:text-xs transition-all ${width === dim.w && height === dim.h ? "border-blue-600 bg-blue-50 text-blue-700 ring-2 ring-blue-100" : "border-gray-200 dark:border-slate-800 bg-white text-gray-500 hover:border-gray-300"}`}
                                                 >
                                                     {dim.label}
                                                 </button>
                                             ))}
                            ...
                            <AccordionStep stepNumber={2} title="Material & Finisaje" summary={summaryStep2} isOpen={activeStep === 2} onClick={() => setActiveStep(2)}>
                                <div className="space-y-4">
                                    ...
                                    <label className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl cursor-pointer">
                                        <input type="checkbox" checked={wantWindHoles} onChange={e => setWantWindHoles(e.target.checked)} className="w-5 h-5 text-blue-600 rounded" />
                                ...
                                <div className="space-y-4">
                                    ...
                                    {designOption === 'upload' && (
                                        <div className="space-y-3">
                                            <label className="flex flex-col items-center justify-center w-full h-32 px-4 transition bg-white border-2 border-gray-300 border-dashed rounded-xl appearance-none cursor-pointer hover:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                            ...
                                            {artworkUrl && <p className="text-blue-600 font-bold text-center text-sm">Grafică proprie recepționată!</p>}
                                        </div>
                                    )}
                                </div>
                            </AccordionStep>
