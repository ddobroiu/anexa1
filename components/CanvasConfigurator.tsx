          ...
          <div className="lg:sticky top-24 h-max space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-800">
                <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center shrink-0"><Frame size={18} /></div>
          ...
          <div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800 px-4">
                  ...
                  {input.frameType === "framed" ? (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 pt-2">
                      {Object.keys(CANVAS_CONSTANTS.FRAMED_PRICES_RECTANGLE).map((size) => (
                        <button key={size} onClick={() => updateInput("framedSize", size)} className={`p-3 text-sm font-bold border-2 rounded-lg transition-all ${input.framedSize === size ? "border-blue-600 bg-blue-50 text-blue-700" : "border-gray-200 bg-white text-gray-500 hover:border-gray-300"}`}>{size}cm</button>
                      ))}
                    </div>
                  ) : (
               ...
               <AccordionStep stepNumber={3} title="Grafică" summary={input.designOption} isOpen={activeStep === 3} onClick={() => setActiveStep(3)} isLast={true}>
                  <div className="grid grid-cols-2 gap-2 pt-2">
                    <button onClick={() => updateInput("designOption", "upload")} className={`p-3 rounded-lg border-2 text-sm ${input.designOption === 'upload' ? 'border-blue-600 bg-blue-50' : 'border-gray-300 bg-white hover:border-gray-400'}`}>Încărcă Poza</button>
                    <button onClick={() => updateInput("designOption", "pro")} className={`p-3 rounded-lg border-2 text-sm ${input.designOption === 'pro' ? 'border-blue-600 bg-blue-50' : 'border-gray-300 bg-white hover:border-gray-400'}`}>Design Pro</button>
                  </div>
