    ...
    >
      ...
      {uploadedItems > 0 && (
         <span className={`text-[10px] font-bold px-1.5 rounded-md min-w-[18px] text-center ${
           isComplete ? "bg-blue-500 text-white" : "bg-amber-500 text-black"
         }`}>
          ...
          {items.map((item, idx) => (
            <div key={idx} className="bg-white/5 p-4 rounded-xl border border-white/5 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    {item.artworkUrl ? (
                        <div className="flex items-center gap-3 bg-blue-500/10 border border-blue-500/20 p-2 rounded-lg w-full sm:w-auto">
                            <div className="hidden sm:flex h-8 w-8 bg-blue-500/20 rounded items-center justify-center">
                                <CheckCircle2 size={16} className="text-blue-400" />
                            </div>
                            <div className="flex-1 min-w-0 mr-2">
                                <p className="text-xs font-medium text-blue-400">Fișier Client</p>
                                <p className="text-[10px] text-blue-400/60 truncate max-w-[150px]">
                                  Încărcat
                                </p>
                            </div>
                            <Button size="sm" className="h-8 bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-900/20">
