...
function FonduriTypeSwitch() {
    ...
    return (
        <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700">
            <Link href="/fonduri-pnrr" className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${isPnrr ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}>PNRR</Link>
            <Link href="/fonduri-regio" className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${isRegio ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}>REGIO</Link>
            <Link href="/fonduri-nationale" className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${isNationale ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}>Naționale</Link>
        </div>
    );
}
            ...
            <div className="container mx-auto px-4 py-10 lg:py-16">
                   ...
                   <div className="lg:sticky top-24 h-max space-y-8 text-slate-900 dark:text-white">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-800">
                            <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center shrink-0"><ShieldCheck size={18} /></div>
                    ...
                    <div>
                           ...
                           <AccordionStep stepNumber={2} title="Afișe & Autocolante" summary={summaryStep2} isOpen={activeStep === 2} onClick={() => setActiveStep(2)}>
                              <div className="py-4 space-y-6">
                                 <div>
                                    ...
                                    <select className="w-full bg-slate-50 dark:bg-slate-800 border-gray-200 dark:border-slate-700 rounded-lg p-2 text-sm focus:border-blue-500" value={selections.afisInformativ} onChange={(e) => setSelections(p => ({ ...p, afisInformativ: e.target.value }))}>
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-3 pb-8 lg:pb-0">
                           <a href="https://wa.me/40750473111" className="h-12 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center gap-2 text-xs font-bold text-slate-900 dark:text-white hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all"><MessageCircle size={16} /> WhatsApp</a>
                           <Link href="/contact" className="h-12 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center gap-2 text-xs font-bold text-slate-900 dark:text-white hover:bg-blue-50 transition-all"><Info size={16} /> Cere Ofertă</Link>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-12 bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800 overflow-hidden">
                    <nav className="border-b border-gray-200 dark:border-slate-800 flex">
                       {["descriere", "faq"].map(t => (
                          <button key={t} onClick={() => setActiveTab(t as any)} className={`flex-1 py-3 text-sm font-medium border-b-2 transition-colors capitalize ${activeTab === t ? 'text-blue-600 border-blue-600 bg-blue-50' : 'text-gray-500 border-transparent hover:bg-slate-50'}`}>{t}</button>
                       ))}
                    </nav>
                       ...
                       {activeTab === 'descriere' && (
                         <div className="prose dark:prose-invert max-w-none text-sm">
                           ...
                           <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 list-none pl-0">
                              <li className="flex items-start gap-2"><Check className="text-blue-600 mt-1 shrink-0" size={16} /> <span><strong>MIV:</strong> Respectăm fonturile, culorile și elementele grafice obligatorii.</span></li>
                              <li className="flex items-start gap-2"><Check className="text-blue-600 mt-1 shrink-0" size={16} /> <span><strong>Echipa Pro:</strong> Expertiza noastră în sute de proiecte auditate.</span></li>
                              <li className="flex items-start gap-2"><Check className="text-blue-600 mt-1 shrink-0" size={16} /> <span><strong>Digital:</strong> Redactare și publicare comunicate pe portaluri cu trafic real.</span></li>
                              <li className="flex items-start gap-2"><Check className="text-blue-600 mt-1 shrink-0" size={16} /> <span><strong>Materiale:</strong> Panouri rezistente UV și plăci din Aluminiu Bond.</span></li>
                           </ul>
                         </div>
                       )}
