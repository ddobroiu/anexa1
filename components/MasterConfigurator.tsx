        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-8 relative z-10 w-full">
            {CONFIGURATORS_REGISTRY.map((config) => (
                <Link
                    ...
                    className="group relative flex flex-col items-center text-center rounded-[2.5rem] border border-slate-200/60 transition-all duration-500 overflow-hidden hover:border-blue-400 hover:shadow-[0_20px_40px_-15px_rgba(16,185,129,0.15)] hover:-translate-y-2 bg-white h-full"
                >
                    ...
                    <div className="p-4 md:p-8 w-full flex-1 flex flex-col items-center justify-center bg-white relative">
                        <h4 className="font-black text-sm md:text-lg leading-tight tracking-tight transition-all duration-300 text-slate-800 dark:text-slate-200 group-hover:text-blue-600 group-hover:scale-105 uppercase italic tracking-tighter">
                            {config.name}
                        </h4>
                        <div className="mt-2 md:mt-4 w-8 h-1 md:h-1.5 bg-slate-100 rounded-full group-hover:w-16 group-hover:bg-blue-500 transition-all duration-700" />
                    </div>
                </Link>
            ))}
        </div>
