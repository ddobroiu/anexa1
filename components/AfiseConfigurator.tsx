...
export default function AfiseConfigurator({ productSlug, initialWidth, initialHeight, productImage, productTitle, productDescription }: { productSlug?: string; initialWidth?: number; initialHeight?: number; productImage?: string; productTitle?: string; productDescription?: string }) {
  ...
  const [activeImage, setActiveImage] = useState<string>((typeof GALLERY !== "undefined" && GALLERY?.[0]) ? GALLERY[0] : ((typeof GALLERY_BASE !== "undefined" && GALLERY_BASE?.[0]) ? GALLERY_BASE[0] : "/placeholder.png"));
                    ...
                    <div className="border-t border-slate-200 pt-3">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className={`flex-1 py-2 px-3 rounded-lg border text-sm font-semibold transition-colors ${designOption === 'upload' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-300 text-gray-700 dark:text-gray-300 hover:bg-slate-50 dark:bg-slate-800'}`}
                        >
                          Încărcare Fișier
                        </button>
                        <button
                          onClick={() => setDesignOption("pro")}
                          className={`flex-1 py-2 px-3 rounded-lg border text-sm font-semibold transition-colors ${designOption === 'pro' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-300 text-gray-700 dark:text-gray-300 hover:bg-slate-50 dark:bg-slate-800'}`}
                        >
                          Vreau Grafică
                        </button>
                      </div>
