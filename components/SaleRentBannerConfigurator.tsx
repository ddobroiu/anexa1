            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div>
                <header className="mb-6">
                  <span className="text-xs font-bold text-blue-600 uppercase tracking-wider">Bannere Reclamă</span>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800 px-4">
                  <AccordionStep stepNumber={1} title="Dimensiune & Tiraj" summary={summaryStep1} isOpen={activeStep === 1} onClick={() => setActiveStep(1)}>
                    <div className="py-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {variants.map((v: any) => (
                                <button key={v.id} onClick={() => setSelectedVariantId(v.id)} className={`px-4 py-2 text-sm font-bold border-2 rounded-lg transition-all ${selectedVariantId === v.id ? "border-blue-600 bg-blue-50 text-blue-700" : "border-gray-200 bg-white text-gray-500 hover:border-gray-300"}`}>{formatSize(v.size)}</button>
                            ))}
                            <Link href="/banner" className="px-4 py-2 text-sm font-bold border-2 border-dashed border-blue-200 bg-blue-50/50 text-blue-600 rounded-lg hover:bg-blue-100 flex items-center gap-2"><Ruler size={16} /><span>Altă Dimensiune?</span></Link>
                        </div>
                        <div className="mt-6">
                  ...
                  <AccordionStep stepNumber={2} title="Material & Finisaje" summary={summaryStep2} isOpen={activeStep === 2} onClick={() => setActiveStep(2)}>
                     ...
                     <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-800 rounded-lg flex justify-between items-center">
                        ...
                        <button onClick={() => setWantWindHoles(!wantWindHoles)} className={`w-12 h-6 rounded-full transition-all relative ${wantWindHoles ? 'bg-blue-500' : 'bg-gray-300'}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${wantWindHoles ? 'right-1' : 'left-1'}`} /></button>
                     </div>
                  </AccordionStep>
