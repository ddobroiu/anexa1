...
function BannerModeSwitchInline() {
  ...
  return (
    <div className="flex flex-wrap gap-1 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700">
      <Link href="/banner" className={`px-3 py-2 rounded-lg text-xs font-bold transition-all ${isFace ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}>O față</Link>
      <Link href="/banner-verso" className={`px-3 py-2 rounded-lg text-xs font-bold transition-all ${isVerso ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}>Față-verso</Link>
      <Link href="/configurator/mesh" className={`px-3 py-2 rounded-lg text-xs font-bold transition-all ${isMesh ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}>Mesh</Link>
    </div>
  );
}
      ...
      <div className="container mx-auto px-4 py-10 lg:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 text-slate-900 dark:text-white">
          <div className="lg:sticky top-24 h-max space-y-8">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800 overflow-hidden">
              <div className="flex border-b border-gray-100 dark:border-slate-800">
                <button onClick={() => setViewMode('gallery')} className={`flex-1 py-3 text-xs font-bold flex items-center justify-center gap-2 transition-all ${viewMode === 'gallery' ? 'text-blue-600 bg-blue-50' : 'text-gray-500 hover:bg-slate-50'}`}><ImageIcon size={16} />Galerie</button>
                <button onClick={() => setViewMode('shape')} className={`flex-1 py-3 text-xs font-bold flex items-center justify-center gap-2 transition-all ${viewMode === 'shape' ? 'text-blue-600 bg-blue-50' : 'text-gray-500 hover:bg-slate-50'}`}><Ruler size={16} />Schiță</button>
              </div>
              ...
              {viewMode === 'gallery' && !artworkUrl && (
                <div className="p-2 grid grid-cols-4 gap-2 border-t border-gray-100 dark:border-slate-800">
                  {galleryImages.map((src, i) => (
                    <button key={src} onClick={() => setActiveIndex(i)} className={`relative rounded-lg aspect-square overflow-hidden transition-all ${activeIndex === i ? "ring-2 ring-blue-500 ring-offset-2" : "hover:opacity-80"}`}><Image src={src} alt="Miniatura" fill className="object-cover" sizes="100px" /></button>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-800">
                 <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center shrink-0"><Layers size={18} /></div>
               ...
               <div className="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-800">
                  <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center shrink-0"><Zap size={18} /></div>
          ...
          <div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800 px-4">
               <AccordionStep stepNumber={1} title="Cote & Cantitate" summary={summaryStep1} isOpen={activeStep === 1} onClick={() => setActiveStep(1)}>
                  <div className="py-4 space-y-6">
                     ...
                     {upsellOpportunity && (
                        <div className="p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-800 flex items-center gap-4 cursor-pointer" onClick={() => updateInput("quantity", upsellOpportunity.requiredQty)}>
                           <div className="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center text-white shrink-0"><TrendingUp size={20} /></div>
                           ...
                           <div className="ml-auto text-blue-600 font-bold text-xs">-{upsellOpportunity.discountPercent}%</div>
                        </div>
                     )}
                  </div>
               </AccordionStep>
               ...
               <AccordionStep stepNumber={2} title="Material & Finisaje" summary={summaryStep2} isOpen={activeStep === 2} onClick={() => setActiveStep(2)}>
                  <div className="py-4 space-y-4">
                     <div className="p-4 rounded-xl border-2 border-blue-600 bg-blue-50 flex items-center gap-4">
                        <div className="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center text-white shrink-0"><ShieldCheck size={20} /></div>
                        <div>
                           <p className="text-sm font-bold text-blue-900">Material Blockout 650g</p>
                           <p className="text-[10px] text-blue-700 uppercase font-bold tracking-widest">Inclus Standard</p>
                        </div>
                     </div>
                  ...
                  <div className="py-4 space-y-6">
                     ...
                     {input.designOption === 'upload' && (
                        <div className={`grid gap-4 ${!input.same_graphic ? 'grid-cols-2' : 'grid-cols-1'}`}>
                           <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-200 dark:border-slate-800 rounded-xl cursor-pointer hover:border-blue-500 transition-all bg-slate-50 dark:bg-slate-900">
                              {uploading ? <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-600"></div> : artworkUrl ? <Check className="text-blue-500" /> : <UploadCloud className="text-gray-400" />}
                           ...
                           {!input.same_graphic && (
                              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-200 dark:border-slate-800 rounded-xl cursor-pointer hover:border-blue-500 transition-all bg-slate-50 dark:bg-slate-900">
                                 {uploading ? <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-600"></div> : artworkUrlVerso ? <Check className="text-blue-500" /> : <UploadCloud className="text-gray-400" />}
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 pb-8 lg:pb-0">
               <a href="https://wa.me/40750473111" className="h-12 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center gap-2 text-xs font-bold text-slate-900 dark:text-white hover:bg-blue-50 transition-all"><MessageCircle size={16} /> WhatsApp</a>
               <Link href="/contact" className="h-12 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center gap-2 text-xs font-bold text-slate-900 dark:text-white hover:bg-blue-50 transition-all"><Info size={16} /> Cerere Ofertă</Link>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800 overflow-hidden">
            <nav className="border-b border-gray-200 dark:border-slate-800 flex">
               {["descriere", "faq"].map(t => (
                  <button key={t} onClick={() => setActiveProductTab(t as any)} className={`flex-1 py-3 text-sm font-medium border-b-2 transition-colors capitalize ${activeProductTab === t ? 'text-blue-600 border-blue-600 bg-blue-50' : 'text-gray-500 border-transparent hover:bg-slate-50'}`}>{t}</button>
               ))}
            </nav>
