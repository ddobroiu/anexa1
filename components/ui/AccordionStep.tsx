...
export const AccordionStep = ({
  ...
  <div className={`relative pl-12 ${premium ? 'premium-step' : ''}`}>
    <div className="absolute top-5 left-0 flex flex-col items-center h-full">
      <span className={`flex items-center justify-center w-8 h-8 rounded-full text-md font-bold transition-all duration-500 ${isOpen ? (premium ? 'bg-amber-600 text-white shadow-[0_0_15px_rgba(217,119,6,0.5)] scale-110' : 'bg-blue-600 text-white shadow-lg') : 'bg-slate-100 text-slate-400'}`}>
        {stepNumber}
      </span>
      {!isLast && <div className={`w-0.5 grow mt-3 rounded-full transition-colors duration-500 ${isOpen ? (premium ? 'bg-amber-600/30' : 'bg-blue-600/20') : 'bg-slate-100'}`}></div>}
    </div>
