...
const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => (
  <Link
    href={href}
    className="relative font-semibold text-slate-800 dark:text-zinc-300 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-300 group py-1"
  >
    {children}
      ...
      {siteConfig.headerNav.map((item: any) =>
        item.highlight ? (
          <Link
            key={item.href}
            href={item.href}
            className="px-5 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-full hover:bg-blue-500 transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5"
          >
            {item.label}
          </Link>
        ) : item.children ? (
          <div key={item.label} className="relative group">
            <button
              className="flex items-center gap-1 font-semibold text-slate-800 dark:text-zinc-300 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-300 group py-4 cursor-pointer outline-none"
              aria-haspopup="true"
              aria-expanded="false"
              ...
              <div className="bg-white/80 dark:bg-zinc-900/90 backdrop-blur-xl rounded-2xl shadow-elevated border border-slate-200/50 dark:border-zinc-800 p-2 overflow-hidden">
                {item.children.map((child: any) => (
                  <Link
                    key={child.href}
                    href={child.href}
                    className="block px-4 py-2.5 rounded-xl text-sm font-semibold text-slate-700 dark:text-zinc-300 hover:bg-blue-50 dark:hover:bg-blue-900/30 hover:text-blue-600 dark:hover:text-blue-400 transition-all"
                  >
                    {child.label}
                  </Link>
                ))}
                  ...
                  {item.children ? (
                    <div className="rounded-lg overflow-hidden">
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2.5 text-left text-sm font-semibold rounded-lg transition-all ${visibleOpenSub === item.label ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600' : 'text-slate-950 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}
                        aria-label={`${visibleOpenSub === item.label ? 'Închide' : 'Deschide'} submeniul ${item.label}`}
                        aria-expanded={visibleOpenSub === item.label}
                      >
                        {item.label}
                        <ChevronDown size={16} className={`transition-transform duration-200 ${visibleOpenSub === item.label ? "rotate-180 text-blue-600" : "text-zinc-400"}`} />
                      </button>
                        ...
                        <div className="overflow-hidden">
                          {item.children.map((child: any) => (
                            <Link
                              ...
                              href={child.href}
                              onClick={onClose}
                              className="flex items-center py-2 pl-8 pr-3 text-sm text-slate-950 dark:text-zinc-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                            >
                              {child.label}
                            </Link>
                          ))}
                    ...
                    <Link
                      href={item.href}
                      onClick={onClose}
                      className={`flex items-center px-3 py-2.5 text-sm font-semibold rounded-lg transition-all ${item.highlight ? 'bg-blue-600 text-white shadow-md mt-3 justify-center hover:bg-blue-700' : 'text-slate-950 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}
                    >
                      {item.label}
                    </Link>
                  )}
  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <a
        ...
        className="hidden md:flex items-center gap-2 px-3 py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-xl border border-blue-100 dark:border-blue-800 transition-all hover:bg-blue-100 dark:hover:bg-blue-800/50 font-black group shadow-sm"
      >
      ...
      {session?.user ? (
        <div className="relative account-dropdown">
          ...
          >
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-[10px] font-black shadow-lg">
            ...
            <ChevronDown size={14} className={`text-zinc-400 transition-transform duration-300 ${isAccountOpen ? 'rotate-180 text-blue-500' : ''}`} />
          </button>
          ...
          <div className={`absolute top-full right-0 w-64 pt-2 transition-all duration-200 ease-out z-60 ${isAccountOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-2 pointer-events-none'
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden ring-1 ring-black/5">
              ...
              <div className="px-6 py-5 border-b border-zinc-100 dark:border-zinc-800 bg-gradient-to-br from-zinc-50/50 to-white dark:from-zinc-800/30 dark:to-zinc-900">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-xl font-bold shadow-blue-500/20 shadow-xl">
                    {(session.user.name?.[0] || session.user.email?.[0] || "U").toUpperCase()}
                  </div>
              ...
              <div className="p-2 space-y-0.5">
                <div className="px-3 pt-2 pb-1 text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest">Activitate</div>

                <Link
                  href="/account?tab=orders"
                  onClick={() => setIsAccountOpen(false)}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 hover:text-blue-600 dark:hover:text-blue-400 transition-all group"
                >
                ...
                <div className="px-3 pt-4 pb-1 text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest">Setări Cont</div>
        ...
        <div className="container mx-auto px-4 sm:px-6 h-16 flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="lg:hidden p-3 bg-blue-600 hover:bg-blue-700 rounded-xl transition-all active:scale-95 shadow-lg"
