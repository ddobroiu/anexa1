...
const FAQItem = ({ question, answer, isOpen, onClick }: { question: string, answer: string, isOpen: boolean, onClick: () => void }) => {
    return (
        <div className="border-b border-slate-100 last:border-0">
            ...
            >
                <span className="text-slate-900 font-bold text-lg group-hover:text-blue-600 transition-colors tracking-tight">
                    {question}
                </span>
                <ChevronDown
...
export function SeoDimensionLanding({ productId, productName, w, h }: SeoDimensionLandingProps) {
    ...
    const heroText = spintax(`{Producem|Realizăm|Tipărim} ${productName.toLowerCase()} la dimensiunea fixă de ${w}x${h} cm {pentru impact maxim|cu precizie milimetrică}. {Fiecare|Orice} ${productName.toLowerCase()} de ${w}x${h} cm este {făcut să reziste|verificat manual} și {gata de montat|livrat rapid} prin Anexa1.`, `${productId}-${w}x${h}`);
    ...
    return (
        <div className="bg-[#fafafc] min-h-screen">
            ...
            <div className="w-full bg-white border-b border-slate-100 overflow-hidden relative">
                <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/4 w-[600px] h-[600px] bg-blue-500/5 blur-[120px] rounded-full" />
                    ...
                    <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-center">
                        <div className="flex-1 order-2 lg:order-1 text-center lg:text-left">
                            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-slate-900 rounded-full text-white font-black text-[10px] uppercase tracking-[0.2em] mb-8 border border-slate-800 shadow-xl">
                                <Ruler size={14} className="text-blue-400" /> Format Standard: {w}x{h} CM
                            </div>
                            <h1 className="text-5xl md:text-8xl font-black text-slate-900 leading-[0.9] tracking-tighter mb-8 uppercase">
                                {productName} <br />
                                <span className="text-blue-500">{w}X{h} CM</span>
                            </h1>
                            <p className="text-lg md:text-xl text-slate-500 font-medium leading-relaxed mb-6 max-w-2xl mx-auto lg:mx-0">
                                {heroText}
                            </p>

                            <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 mb-10 flex items-start gap-4 max-w-xl mx-auto lg:mx-0 text-left">
                                <div className="p-2 bg-blue-500 text-white rounded-lg shrink-0">
                                    <Sparkles size={18} />
                                </div>
                                <div>
                                    <h4 className="text-blue-900 font-black text-sm uppercase tracking-tight leading-none mb-1">Dimensiune Personalizată?</h4>
                                    <p className="text-blue-700 text-xs font-medium leading-tight">Putem realiza acest produs la absolut <strong className="font-black">ORICE</strong> dimensiune ai nevoie, chiar dacă nu este în tabelul de mai jos.</p>
                                </div>
                            </div>
                            <div className="flex flex-col sm:flex-row gap-4 mb-12 justify-center lg:justify-start">
                                <button onClick={onStartConfig} className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-black text-lg hover:bg-blue-500 transition-all flex items-center justify-center gap-3 shadow-2xl active:scale-95 group">
                                    <Zap size={22} className="fill-current" /> CONFIGUREAZĂ ONLINE
                                </button>
                                <a href={`https://wa.me/40750473111?text=Bună%20ziua,%20mă%20interesează%20${encodeURIComponent(productName)}%20la%20dimensiunea%20${w}x${h}%20cm`} target="_blank" className="px-10 py-5 bg-white text-slate-900 border-2 border-slate-200 rounded-2xl font-black text-lg hover:border-blue-500 hover:text-blue-600 transition-all flex items-center justify-center gap-3 shadow-sm active:scale-95">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                                {highlights.map((h, i) => (
                                    <div key={i} className="flex flex-col gap-1">
                                        <div className="flex items-center gap-1.5 text-blue-600 font-black text-[10px] uppercase tracking-wider text-left lg:text-left">
                        ...
                        <div className="flex-1 order-1 lg:order-2 w-full max-w-[600px] lg:max-w-none">
                            <div className="relative group flex items-center justify-center">
                                <div className="absolute inset-0 bg-gradient-to-tr from-blue-500/10 to-blue-500/5 rounded-full blur-[100px] opacity-60" />
                ...
                <div className="flex flex-col md:flex-row gap-16 items-center">
                    <div className="flex-1">
                        <h2 className="text-3xl md:text-6xl font-black text-slate-900 mb-8 tracking-tighter uppercase leading-[0.95]">
                            DETALII TEHNICE <br/> {productName} <span className="text-blue-500">{w}X{h} CM</span>
                        </h2>
                    ...
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
                        ...
                        <div className="bg-slate-950 p-8 rounded-[2.5rem] border border-slate-800 shadow-xl flex flex-col justify-between aspect-square md:aspect-auto md:h-64">
                            ...
                            <div className="text-left">
                                <h3 className="text-xl font-black text-white uppercase mb-2 leading-none tracking-tight text-blue-400">Garanție Anexa1</h3>
                ...
                <div className="max-w-7xl mx-auto px-4">
                    <div className="text-center mb-16">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-50 border border-slate-100 text-slate-500 text-[10px] font-black uppercase tracking-widest mb-4">
                            <Zap size={14} className="text-blue-500" /> EXPLOREAZĂ TOATĂ GAMA
                        </div>
            ...
            <div className="bg-white py-24 border-t border-slate-100">
                <div className="max-w-4xl mx-auto px-4">
                    <div className="text-center mb-16">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-50 border border-slate-100 text-slate-500 text-[10px] font-black uppercase tracking-widest mb-4">
                            <HelpCircle size={14} className="text-blue-500" /> SUPORT CONFIGURARE {w}x{h}
                        </div>
                ...
                <div className="max-w-7xl mx-auto px-4 text-left lg:text-center">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2 md:gap-3">
                        {[
                            ...
                            <Link key={i} href={`/configurator/${productId}-${dim.w}x${dim.h}`} className="px-2 py-3 bg-white border border-slate-200 rounded-xl text-slate-600 hover:text-blue-600 hover:border-blue-500 transition-all text-[10px] md:text-xs font-black text-center shadow-sm uppercase tracking-tighter">
                                {dim.w}x{dim.h} CM
                            </Link>
                        ))}
                    </div>
            ...
            <div className="bg-white py-24 border-t border-slate-100 relative overflow-hidden">
                <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <button onClick={onStartConfig} className="px-12 py-6 bg-slate-950 text-white rounded-2xl font-black text-xl hover:bg-blue-600 transition-all flex items-center justify-center gap-3 shadow-2xl active:scale-95"> <Zap size={24} fill="white" /> CONFIGURATOR </button>
