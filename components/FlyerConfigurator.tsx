      ...
      <div className="container mx-auto px-4 py-10 lg:py-16">
              ...
              <div className="p-2 grid grid-cols-4 gap-2">
                {GALLERY.map((src, i) => (
                  <button 
                    ...
                    className={`relative rounded-lg aspect-square overflow-hidden ${activeIndex === i ? "ring-2 ring-offset-2 ring-blue-500" : "hover:opacity-80"}`}
                  >
                    <Image 
                      src={src} 
                      alt="Miniatura" 
          ...
          <div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800 px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-2">
                      {FLYER_CONSTANTS.SIZES.map((s: any) => <button key={s.key} onClick={() => setSizeKey(s.key)} className={`p-3 rounded-lg border-2 text-sm ${sizeKey === s.key ? 'border-blue-600 bg-blue-50' : 'border-gray-300 bg-white hover:border-gray-400'}`}>{s.label}</button>)}
                    </div>
                  </div>
                  <div>
                    <label className="field-label">Hârtie</label>
                    <div className="grid grid-cols-1 gap-2 mt-2">
                      {FLYER_CONSTANTS.PAPER_WEIGHTS.map((p: any) => <button key={p.key} onClick={() => setPaperWeightKey(p.key)} className={`p-3 rounded-lg border-2 text-sm ${paperWeightKey === p.key ? 'border-blue-600 bg-blue-50' : 'border-gray-300 bg-white hover:border-gray-400'}`}>{p.label}</button>)}
                    </div>
                  </div>
                </div>
              </AccordionStep>
              ...
              <AccordionStep stepNumber={2} title="Tiraj & Față/Dos" summary={summary2} isOpen={activeStep === 2} onClick={() => setActiveStep(2)}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <NumberInput label="Cantitate (buc)" value={quantity} onChange={setQty} />
                  <div>
                    <label className="field-label">Imprimare</label>
                    <div className="mt-2 grid grid-cols-2 gap-2">
                      <button onClick={() => setTwoSided(false)} className={`p-3 rounded-lg border-2 text-sm ${!twoSided ? 'border-blue-600 bg-blue-50' : 'border-gray-300 bg-white hover:border-gray-400'}`}>Față</button>
                      <button onClick={() => setTwoSided(true)} className={`p-3 rounded-lg border-2 text-sm ${twoSided ? 'border-blue-600 bg-blue-50' : 'border-gray-300 bg-white hover:border-gray-400'}`}>Față-verso</button>
                    </div>
                  </div>
                </div>

                {upsellOpportunity && (
                  <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-lg cursor-pointer hover:bg-amber-100 transition-colors" onClick={() => setQty(upsellOpportunity.requiredQty)}>
              ...
              <AccordionStep stepNumber={3} title="Grafică" summary={designOption === 'upload' ? 'Grafică proprie' : 'Vreau grafică'} isOpen={activeStep === 3} onClick={() => setActiveStep(3)} isLast={true}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  <button onClick={() => setDesignOption('upload')} className={`p-3 rounded-lg border-2 text-sm ${designOption === 'upload' ? 'border-blue-600 bg-blue-50' : 'border-gray-300 bg-white hover:border-gray-400'}`}>Am grafică</button>
                  <button onClick={() => setDesignOption('pro')} className={`p-3 rounded-lg border-2 text-sm ${designOption === 'pro' ? 'border-blue-600 bg-blue-50' : 'border-gray-300 bg-white hover:border-gray-400'}`}>Vreau grafică ({formatMoneyDisplay(priceData.proFee || 0)})</button>
                </div>
        ...
        <div className="mt-8 lg:mt-12 bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800">
          <nav className="border-b border-gray-200 dark:border-slate-800 flex">
            <button onClick={() => setActiveProductTab('descriere')} className={`px-4 py-3 font-medium text-sm transition-colors border-b-2 ${activeProductTab === 'descriere' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-300'}`}>Descriere</button>
            <button onClick={() => setActiveProductTab('recenzii')} className={`px-4 py-3 font-medium text-sm transition-colors border-b-2 ${activeProductTab === 'recenzii' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-300'}`}>Recenzii</button>
            <button onClick={() => setActiveProductTab('faq')} className={`px-4 py-3 font-medium text-sm transition-colors border-b-2 ${activeProductTab === 'faq' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-300'}`}>FAQ</button>
          </nav>
