        ...
        <div className="space-y-8">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm">
                <h2 className="text-xl font-bold mb-6 text-slate-900 dark:text-white flex items-center gap-2">
                    <span className="w-8 h-8 rounded-lg bg-blue-600/10 text-blue-600 flex items-center justify-center text-sm font-black">+</span>
                ...
                <div className="flex w-full overflow-hidden border border-slate-200 dark:border-slate-800 rounded-xl mb-6 p-1 bg-slate-50 dark:bg-slate-950 max-w-sm">
                    <button type="button" onClick={() => onForm("type", "shipping")} className={`flex-1 rounded-lg px-4 py-2 text-sm font-bold transition-all duration-200 ${form.type === "shipping" ? "bg-white dark:bg-slate-800 text-blue-600 shadow-sm" : "text-slate-500"}`}>Livrare</button>
                    <button type="button" onClick={() => onForm("type", "billing")} className={`flex-1 rounded-lg px-4 py-2 text-sm font-bold transition-all duration-200 ${form.type === "billing" ? "bg-white dark:bg-slate-800 text-blue-600 shadow-sm" : "text-slate-500"}`}>Facturare</button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="md:col-span-4 flex items-center justify-between mt-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                        <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
                            <input type="checkbox" checked={!!form.isDefault} onChange={(e) => onForm("isDefault", e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                            <span className="text-slate-600 dark:text-slate-400 font-medium">Setează ca implicită</span>
                        </label>
                        <button onClick={saveNew} disabled={saving} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl font-bold transition-all shadow-lg shadow-blue-600/20 disabled:opacity-50">
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-3 mb-2">
                                                ...
                                                {a.isDefault && <span className="px-2 py-0.5 bg-blue-500/10 text-blue-600 text-[10px] font-black uppercase tracking-widest rounded-md border border-blue-500/20">Implicit</span>}
                                                <span className={`px-2 py-0.5 text-[10px] font-black uppercase tracking-widest rounded-md border ${a.type === 'billing' ? 'bg-orange-500/10 text-orange-600 border-orange-500/20' : 'bg-blue-500/10 text-blue-600 border-blue-500/20'}`}>
                                        ...
                                        <div className="flex items-center gap-1.5 self-start">
                                            {a.type === 'shipping' && !a.isDefault && <button className="px-3 py-1.5 text-xs font-black uppercase text-blue-600 hover:bg-blue-600 hover:text-white rounded-lg border border-blue-600/20 transition-all" onClick={() => update(a.id!, { isDefault: true })}>Setează Implicit</button>}
...
function EditRow({ a, onCancel, onSave }: { a: Address; onCancel: () => void; onSave: (patch: Partial<Address>) => void }) {
    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-blue-100 dark:border-blue-900/30 grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="md:col-span-2">
            ...
            <div className="md:col-span-4 flex justify-end gap-2 mt-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                <button className="bg-blue-600 text-white px-6 py-2 rounded-xl font-bold shadow-lg" onClick={submit} disabled={saving}>{saving ? '...' : 'Salvează'}</button>
            </div>
        </div>
    );
}
...
function inputCls(hasError?: string, disabled?: boolean) {
    return `w-full rounded-xl border px-4 py-2.5 focus:outline-none focus:ring-2 bg-white dark:bg-slate-800 text-slate-900 dark:text-white border-slate-200 dark:border-slate-800 focus:ring-blue-500 placeholder-slate-400 font-medium transition-all ${hasError ? "border-red-500 ring-1 ring-red-500/20" : ""} ${disabled ? "opacity-50 cursor-not-allowed" : ""}`;
}
