            ...
            {activeTool === 'upload' && (
                <div className="space-y-4">
                    ...
                    <button
                        ...
                        className="w-full py-3 bg-blue-600 text-white rounded-lg font-bold flex items-center justify-center gap-2 hover:bg-blue-700 transition"
                    >
            ...
            {activeTool === 'library' && (
                <div className="space-y-4">
                    ...
                    <form onSubmit={handlePixabaySearch} className="relative">
                        <input
                            type="text"
                            className="w-full pl-3 pr-10 py-2 border rounded-lg text-sm bg-slate-50 dark:bg-slate-800 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition"
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 hide-scrollbar">
                        <label className="flex items-center gap-2 text-xs bg-slate-100 px-2 py-1 rounded cursor-pointer shrink-0 hover:bg-slate-200">
                            <input type="checkbox" checked={pixabayTransparent} onChange={(e) => setPixabayTransparent(e.target.checked)} className="rounded text-blue-600 focus:ring-blue-500" />
                            Transparent
                        </label>
                        <select
                    ...
                    {pixabayResults.length > 0 && !isSearching && (
                        <button onClick={handleLoadMore} className="w-full py-2 text-sm text-blue-600 font-semibold hover:bg-slate-50 dark:bg-slate-800 rounded mt-2">
                            Mai multe...
                        </button>
                    )}
                </div>
            ...
            {activeTool === 'elements' && (
                <div className="space-y-4">
                    ...
                    {vectorResults.length > 0 && !isSearchingVectors && (
                        <button onClick={handleLoadMoreVectors} className="w-full py-2 text-sm text-blue-600 font-semibold hover:bg-slate-50 dark:bg-slate-800 rounded mt-2">
                            Mai multe...
                        </button>
                    )}
                </div>
            ...
            {activeTool === 'bg' && (
                <div className="space-y-4">
                    ...
                    <button
                        ...
                        className="w-full py-2 border-2 border-dashed border-slate-300 rounded-lg text-slate-500 hover:text-blue-600 hover:border-blue-600 transition flex items-center justify-center gap-2"
                    >
                        <ImagePlus size={18} /> Încarcă Fundal Propriu
                    </button>

                    <div className="grid grid-cols-3 gap-2 mt-4">
                        <button onClick={() => setBackground('#ffffff')} className="aspect-square bg-white border border-slate-200 rounded hover:ring-2 ring-blue-500" title="White"></button>
                        <button onClick={() => setBackground('#000000')} className="aspect-square bg-black border border-slate-200 rounded hover:ring-2 ring-blue-500" title="Black"></button>
                        {/* Simple colors */}
                        {['#ef4444', '#f97316', '#eab308', '#22c55e', '#3b82f6', '#a855f7', '#ec4899', '#64748b'].map(c => (
                            <button key={c} onClick={() => setBackground(c)} className="aspect-square rounded hover:ring-2 ring-blue-500 border border-transparent" style={{ background: c }}></button>
                        ))}
                    </div>
                </div>
            )}
