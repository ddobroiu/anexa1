...
/** Ad/analytics consent signals, granted only after the user accepts. */
const AD_CONSENT_KEYS = [
    "ad_storage",
    "ad_user_data",
    "ad_personalization",
    "analytics_storage",
...
/** Consent Mode defaults (set in app/layout.tsx) start denied — push the user's choice. */
function updateGoogleConsent(state: "granted" | "denied") {
    ...
    gtag("consent", "update", Object.fromEntries(AD_CONSENT_KEYS.map((k) => [k, state])));
}
                ...
                <div className="fixed bottom-0 left-0 right-0 z-[99999] p-4 sm:p-6 md:max-w-md mx-auto pointer-events-none">
                    <div className="bg-white/95 backdrop-blur-xl border border-slate-200 shadow-2xl p-6 rounded-3xl pointer-events-auto">
                        ...
                        <p className="text-sm text-slate-500 mb-6 leading-relaxed">
                            Ne ajută să înțelegem cum e folosit site-ul și să-ți arătăm oferte relevante. <Link href="/politica-cookies" className="text-blue-600 hover:underline">Detalii</Link>.
                        </p>
                        <div className="flex flex-col sm:flex-row gap-3">
                            <button
                                onClick={handleAccept}
                                className="flex-1 px-4 py-2.5 rounded-xl block bg-blue-600 text-white font-bold hover:bg-blue-500 shadow-lg shadow-blue-600/20 transition-all active:scale-95 text-sm"
                            >
                                Accept
                            </button>
                            <button
