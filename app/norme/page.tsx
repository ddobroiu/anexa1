...
export const metadata: Metadata = {
  ...
  alternates: { canonical: 'https://www.anexa1.ro/norme' }
};
                ...
                <div className="container mx-auto px-6 relative z-10">
                    <div className="max-w-4xl mx-auto lg:mx-0">
                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-slate-900 text-white rounded-lg font-black text-[10px] uppercase tracking-widest mb-6 italic shadow-xl">
                            <Scale size={14} className="text-blue-500" /> DEPARTAMENT CONFORMITATE
                        </div>
                        <h1 className="text-5xl md:text-8xl font-black text-slate-900 tracking-tighter leading-[0.9] mb-8 uppercase italic">
                            Semnalistică <br /> <span className="text-blue-500 not-italic uppercase tracking-tighter">Legislație & Norme.</span>
                        </h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8 lg:gap-12">
                    {REGLEMENTARI_DATA.map((reg) => (
                        <Link
                            key={reg.id}
                            href={`/norme/${reg.slug}`}
                            className="group bg-white rounded-[4rem] border border-slate-100 p-12 hover:shadow-2xl hover:border-blue-200 transition-all duration-700 hover:-translate-y-2 flex flex-col justify-between relative overflow-hidden h-full"
                        >
                            <span className="absolute top-0 right-0 py-2 px-6 bg-slate-50 text-[10px] font-black uppercase text-slate-300 tracking-[0.3em] group-hover:bg-blue-500 group-hover:text-white transition-all italic">
                                {reg.lawReference}
                            </span>
                            
                            <div className="flex-1">
                                <div className="w-16 h-16 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 mb-8 border border-slate-50 group-hover:bg-slate-950 group-hover:text-blue-400 transition-all shadow-sm">
                                    <FileText size={32} strokeWidth={2.5} />
                                </div>
                                <h3 className="text-3xl font-black text-slate-900 group-hover:text-blue-600 transition-colors mb-4 uppercase tracking-tighter italic">
                                    {reg.name}
                                </h3>
            ...
            <div className="py-24 bg-slate-50">
                <div className="container mx-auto px-6">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-12 text-center">
                        ...
                        <div className="flex flex-col items-center">
                            <Scale className="text-blue-500 mb-4" size={48} />
