        ...
        <main className="container py-24 min-h-screen">
            <header className="mb-16 border-b border-slate-100 pb-16">
                <h1 className="text-4xl md:text-6xl font-black text-slate-900 mb-8 leading-[1.1] tracking-tighter">
                    Harta Site <br /><span className="text-blue-500">{siteName}</span>
                </h1>
            ...
            <div className="grid md:grid-cols-2 gap-16">
                ...
                <section>
                    ...
                    <ul className="flex flex-col gap-3">
                        <li><Link href="/" className="text-blue-600 hover:text-blue-800 font-semibold underline underline-offset-4">Acasă</Link></li>
                        <li><Link href="/shop" className="text-blue-600 hover:text-blue-800 font-semibold underline underline-offset-4">Toate Produsele</Link></li>
                        <li><Link href="/contact" className="text-blue-600 hover:text-blue-800 font-semibold underline underline-offset-4">Contact</Link></li>
                        <li><Link href="/termeni" className="text-blue-600 hover:text-blue-800 font-semibold underline underline-offset-4">Termeni și Condiții</Link></li>
                        <li><Link href="/politica-cookies" className="text-blue-600 hover:text-blue-800 font-semibold underline underline-offset-4">Politica de Cookie-uri</Link></li>
                    </ul>
                </section>
                    ...
                    <div className="flex flex-col gap-8">
                        {Array.from(categoriesMap.entries()).slice(0, 10).map(([cat, prods]) => (
                            <div key={cat}>
                                ...
                                <ul className="flex flex-col gap-2 pl-4 border-l-2 border-blue-100">
                                    {prods.slice(0, 15).map(p => (
                                        <li key={p.id}>
                                            <Link href={p.routeSlug || `/shop/${cat}/${p.slug}`} className="text-slate-600 hover:text-blue-600 text-sm">
                                                {p.title}
                                            </Link>
                                        </li>
                                    ))}
                ...
                <section className="md:col-span-2 mt-8">
                    ...
                    <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                        {JUDETE_FULL_DATA.map(j => (
                            <li key={j.slug}>
                                <Link href={`/judet/${j.slug}`} className="text-sm font-semibold text-slate-700 hover:text-blue-600 hover:underline">
                                    Județul {j.name}
                                </Link>
                            </li>
                        ))}
