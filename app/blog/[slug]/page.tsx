...
export async function generateMetadata({ params }: PageProps) {
    ...
    return {
        title: `${post.title} | Blog Anexa1.ro`,
        ...
        <main className="min-h-screen pt-24 pb-20 bg-white">
            ...
            <div className="container mx-auto px-4 md:px-8">
                ...
                <div className="max-w-3xl mx-auto mb-8">
                    <Link href="/blog" className="inline-flex items-center text-sm font-bold text-slate-400 hover:text-blue-600 transition-colors">
                ...
                <article className="max-w-3xl mx-auto">
                    ...
                    <header className="mb-10">
                        <div className="flex gap-2 mb-4">
                            {post.tags.map((tag) => (
                                <span key={tag} className="text-[10px] font-black uppercase tracking-widest text-blue-600 bg-blue-50 px-2 py-1 rounded-md">
                                    #{tag}
                                </span>
                            ))}
                        </div>
                    ...
                    {/* Content */}
                    <div
                        className="prose prose-slate lg:prose-xl prose-headings:font-black prose-headings:text-slate-900 prose-p:text-slate-600 prose-p:leading-relaxed prose-a:text-blue-600 prose-a:font-bold prose-a:no-underline hover:prose-a:underline"
                        dangerouslySetInnerHTML={{ __html: post.contentHtml }}
                    />

                    {/* CTA SECTION */}
                    <div className="mt-16 p-8 md:p-12 bg-slate-900 rounded-[2rem] text-white overflow-hidden relative group">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-blue-600/20 rounded-full blur-[80px] -mr-32 -mt-32 pointer-events-none group-hover:bg-blue-600/30 transition-colors"></div>
                            ...
                            <div className="flex flex-wrap justify-center gap-3">
                                {[
                                    ...
                                    <Link
                                        key={cta.href}
                                        href={cta.href}
                                        className="px-6 py-3 bg-white/10 hover:bg-blue-600 text-white rounded-xl font-bold transition-all hover:scale-105"
                                    >
                                        {cta.label}
                                    </Link>
                                ))}
                                <Link
                                    href="/shop"
                                    className="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold transition-all hover:scale-105 shadow-lg shadow-blue-600/20"
                                >
                                    Toate produsele
                                </Link>
                            </div>
