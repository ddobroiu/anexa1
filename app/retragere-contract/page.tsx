    ...
    async function handleSubmit(e: FormEvent<HTMLFormElement>) {
        ...
        try {
            const response = await fetch("/api/retur", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ ...form, products, source: "anexa1.ro" }),
            });
            ...
            <div className="container mx-auto max-w-2xl">
                <div className="text-center mb-12">
                    <h1 className="text-3xl lg:text-5xl font-black text-slate-900 dark:text-white mb-4 tracking-tighter uppercase italic leading-none">
                        Retrage-te din <span className="text-blue-600">contract</span>
                    </h1>
                    <div className="h-1.5 w-24 bg-blue-600 mx-auto rounded-full shadow-lg mb-6"></div>
                    <p className="text-slate-600 dark:text-slate-400 max-w-xl mx-auto">
                        Formularul de mai jos transmite o cerere oficială de retragere dintr-un contract la distanță, conform legii.
                        Vezi și <Link href="/politica-retur" className="text-blue-600 font-semibold underline">politica noastră de retur</Link> pentru condiții și termene.
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl border border-slate-200 dark:border-slate-800 p-8 space-y-5">
                    ...
                    <label className="flex items-start gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 cursor-pointer">
                        <input type="checkbox" checked={confirmed} onChange={(e) => setConfirmed(e.target.checked)} className="mt-1 w-5 h-5 rounded border-slate-300 text-blue-600" />
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full py-4 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl font-black uppercase tracking-widest transition-colors"
                    >
