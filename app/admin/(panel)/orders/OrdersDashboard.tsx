...
function StatusBadge({ status }: { status: string }) {
  const configs: Record<string, { label: string; class: string; dot: string }> = {
    ...
    fulfilled: { 
      ...
      dot: "bg-blue-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" 
    },
    canceled: { 
      label: "Anulată", 
...
function ClientAvatar({ name, source }: { name: string; source?: string | null }) {
  ...
  let colorClass = 'from-blue-500 to-teal-600'; // Default
  
  if (source === 'prynt.ro') colorClass = 'from-purple-500 to-indigo-600';
  else if (source === 'tablou.net') colorClass = 'from-orange-500 to-rose-600';
  ...
  return (
    <div className="flex flex-col gap-1 w-24">
      <div className="flex justify-between text-[9px] font-black uppercase text-zinc-500">
        <span>Grafică</span>
        <span className={ready === total ? "text-blue-400" : "text-amber-400"}>{ready}/{total}</span>
      </div>
      <div className="h-1 w-full bg-zinc-800 rounded-full overflow-hidden border border-white/5">
        <motion.div 
          initial={{ width: 0 }}
          animate={{ width: `${percentage}%` }}
          className={cn("h-full rounded-full transition-all duration-500", ready === total ? "bg-blue-500 shadow-[0_0_8px_rgba(16,185,129,0.3)]" : "bg-amber-500")}
        />
      </div>
    </div>
  );
...
function SourceBadge({ source }: { source?: string | null }) {
  ...
  const clean = source.replace('.ro', '').replace('anexa1', 'Shop').replace('prynt', 'Prynt').replace('.net', '');
  
  let colorStyle = "bg-blue-500/10 text-blue-400 border-blue-500/20"; // anexa1/default
        ...
        <div className="max-w-[1920px] mx-auto flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="flex items-center gap-4 w-full md:w-auto">
             <div className="relative group flex-1 md:w-96">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500 w-4 h-4 group-focus-within:text-blue-500 transition-colors" />
                <input
                  type="text"
                  placeholder="Caută în comenzi (Nr, Nume, Email)..."
                  className="w-full bg-zinc-900/50 border border-white/10 rounded-2xl py-2.5 pl-11 pr-4 text-sm text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/40 transition-all"
                  value={searchTerm}
          ...
          <div className="hidden md:flex items-center gap-3">
             ...
             <div className="relative group">
                ...
                <select 
                  className="bg-zinc-900/50 border border-white/10 rounded-2xl py-2 pl-9 pr-8 text-xs font-bold text-zinc-300 focus:outline-none focus:border-blue-500/40 appearance-none cursor-pointer"
                  value={sourceFilter}
                  onChange={(e) => setSourceFilter(e.target.value)}
                >
                  <option value="all">Sursă: Toate</option>
                  <option value="anexa1.ro">Anexa1</option>
                  <option value="prynt.ro">Prynt</option>
                  <option value="euprint.ro">EuPrint</option>
                  <option value="tablou.net">Tablou.net</option>
             ...
             <div className="flex bg-zinc-900/50 border border-white/10 rounded-2xl p-1">
                <button 
                  ...
                  className={cn("p-1.5 rounded-xl transition-all", viewMode === 'list' ? "bg-blue-500 text-white shadow-lg shadow-blue-500/20" : "text-zinc-500 hover:text-white")}
                >
                  <List size={16} />
                </button>
                <button 
                  onClick={() => setViewMode('grid')}
                  className={cn("p-1.5 rounded-xl transition-all", viewMode === 'grid' ? "bg-blue-500 text-white shadow-lg shadow-blue-500/20" : "text-zinc-500 hover:text-white")}
                >
                  <LayoutGrid size={16} />
                </button>
             </div>
                  ...
                  >
                    ...
                    <td colSpan={6} className="absolute inset-0 p-0 pointer-events-none">
                      <div className={cn(
                        ...
                        isSelected 
                          ? "bg-zinc-800/80 border-blue-500/50 shadow-2xl shadow-blue-500/10 scale-[1.01]" 
                    ...
                    <td className="relative py-5">
                      <div className="flex flex-col">
                        <div className="flex items-center gap-1.5">
                          {isCard || isOP ? <CreditCard className="w-3.5 h-3.5 text-blue-400" /> : <Banknote className="w-3.5 h-3.5 text-amber-500" />}
                          <span className="text-sm font-black text-white">{formatMoney(order.total)}</span>
                        </div>
                        <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mt-0.5">{isCard ? 'Plătit' : isOP ? 'OP (transfer)' : 'Ramburs'}</span>
                      </div>
                    </td>
                    ...
                    <td className="relative py-5 pr-8 text-right">
                       <div className="flex items-center justify-end gap-2">
                          ...
                          <button className="p-2 rounded-xl bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 transition-all active:scale-95">
              ...
              <div className="p-6 border-b border-white/5 flex items-center justify-between bg-zinc-900/40">
                <div className="flex items-center gap-4">
                 ...
                 <div className="flex items-center gap-3">
                   ...
                   <div className="px-3 py-1.5 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-black uppercase tracking-widest">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-thin scrollbar-thumb-zinc-800">
                ...
                <div className="grid grid-cols-3 gap-4">
                   <div className="p-4 rounded-[2rem] bg-zinc-900/50 border border-white/5 flex flex-col items-center justify-center text-center">
                      ...
                      <div className="text-white font-bold text-sm flex items-center gap-1.5">
                        {selectedOrder.paymentMethod === 'Card' ? <CreditCard size={16} className="text-blue-500" /> : <Banknote size={16} className="text-amber-500" />}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-4">
                    <h3 className="text-xs font-black text-zinc-500 uppercase tracking-[0.2em] border-l-2 border-blue-500 pl-3">Client</h3>
                      ...
                      return (
                        <div key={item.id} className="bg-zinc-900/40 rounded-[2rem] p-5 border border-white/5 hover:border-white/10 transition-all flex flex-col gap-4">
                           ...
                           <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                 {isReady ? (
                                   <div className="flex items-center gap-1.5 px-3 py-1 bg-blue-500/10 text-blue-400 rounded-full text-[10px] font-black uppercase tracking-wider border border-blue-500/20 shadow-[0_0_10px_rgba(16,185,129,0.1)]">
                              ...
                              <div className="flex items-center gap-2">
                                 {item.artworkUrl && (
                                   <a 
                                     ...
                                     target="_blank"
                                     className="flex items-center gap-2 px-4 py-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-blue-500/20 active:scale-95"
                                   >
                      ...
                      <div className="col-span-2 p-1.5 bg-zinc-900 border border-white/10 rounded-[2.5rem] flex items-center justify-between">
                         ...
                         <div className="flex gap-1">
                            <button 
                              ...
                              className="h-12 px-6 rounded-[2rem] bg-blue-500 hover:bg-blue-600 text-white font-black text-xs uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50"
                            >
