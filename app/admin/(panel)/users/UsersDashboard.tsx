      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl shadow-sm border border-gray-100">
        <div className="relative group w-full sm:w-96">
          ...
          <input
            ...
            className="pl-10 pr-4 min-h-11 sm:min-h-10 sm:py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none w-full transition-all bg-slate-50 focus:bg-white touch-manipulation"
            value={searchTerm}
              ...
              >
                <div className="flex flex-col lg:flex-row gap-4 sm:gap-5 lg:gap-6 items-start lg:items-center justify-between">
                  ...
                  <div className="flex items-start gap-3 sm:gap-4 flex-1 min-w-0">
                    <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 shrink-0 font-bold text-base sm:text-lg border border-blue-100">
                      {user.name ? user.name.charAt(0).toUpperCase() : user.email.charAt(0).toUpperCase()}
                    </div>
                    <div className="min-w-0 flex-1">
                      <h3 className="font-bold text-slate-900 text-sm sm:text-base truncate flex items-center gap-2">
                        {user.name || "Fără Nume"}
                        {user.emailVerified && <ShieldCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-500 shrink-0" aria-label="Email Verificat" role="img" />}
                      </h3>
                  ...
                  <div className="grid grid-cols-3 gap-4 sm:flex sm:items-center sm:gap-6 w-full lg:w-auto border-t lg:border-t-0 border-gray-100 pt-3 sm:pt-4 lg:pt-0">
                    <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2 mt-0.5">
                            <Package className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-500 shrink-0" />
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="font-bold text-sm sm:text-base text-blue-600 mt-0.5">{formatMoney(totalSpent)}</span>
                    </div>
