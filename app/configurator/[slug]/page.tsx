...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    if (!parsed) {
        return {
            ...
            description: "Personalizează produsul tău la dimensiunea dorită cu Anexa1."
        };
    }

    if (parsed.intent) {
        const title = `${parsed.productName} ${parsed.intentLabel}`;
        const description = `Comandă ${parsed.productName} pentru ${parsed.intentLabel}. Modele gata de personalizat, print premium și livrare rapidă prin Anexa1.ro.`;
        return {
            title,
            description,
            keywords: [parsed.productId, parsed.intent, parsed.intentLabel, 'print personalizat', 'anexa1'],
        };
    }

    const title = `${parsed.productName} ${parsed.w}x${parsed.h}`;
    const description = `Comandă ${parsed.productName} la dimensiunea fixă de ${parsed.w}x${parsed.h} cm. Producător direct, preț calculat instant, print HD rezistent UV și livrare rapidă în 2-4 zile lucrătoare. Personalizează acum formatul ${parsed.w}x${parsed.h} cm pe Anexa1.ro!`;
