...
export default function CheckoutPage() {
  ...
  const isCardDisabled = totalWithShipping < MIN_CARD_PAYMENT;
  ...
  useEffect(() => {
    if (isRambursDisabled && paymentMethod === "cash_on_delivery") {
      setPaymentMethod(isCardDisabled ? "bank_transfer" : "card");
    }
  }, [isRambursDisabled, isCardDisabled, paymentMethod]);

  useEffect(() => {
    if (isCardDisabled && paymentMethod === "card") {
      setPaymentMethod(isRambursDisabled ? "bank_transfer" : "cash_on_delivery");
    }
  }, [isCardDisabled, isRambursDisabled, paymentMethod]);
        ...
        <div className="grid lg:grid-cols-[minmax(0,2fr)_minmax(0,1.25fr)] gap-6 lg:gap-8">
            ...
            <header className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 sm:p-6 shadow-sm">
              <div>
                <h1 className="text-xl sm:text-2xl font-black text-black dark:text-white flex items-center gap-2">
                  Finalizare Comandă
                  <span className="inline-flex items-center justify-center rounded-full bg-blue-600 text-white text-xs font-bold px-3 py-1 shadow-sm">
          ...
          <aside className="lg:col-span-1 lg:sticky lg:top-6 space-y-4 lg:space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
              <div className="bg-gradient-to-r from-teal-600 to-blue-600 px-4 sm:px-5 py-3">
                ...
                <div className="grid gap-3">
                  {!isCardDisabled && (
                    <label
                      className={`flex items-center gap-3 rounded-xl border-2 p-3 cursor-pointer transition ${paymentMethod === "card"
                        ? "border-blue-600 bg-blue-50 dark:bg-blue-950 ring-2 ring-blue-200 dark:ring-blue-800"
                        : "border-slate-300 dark:border-slate-700 hover:border-blue-400 hover:bg-slate-50/60 dark:hover:bg-slate-900/60"
                        }`}
                    >
                      <input
                        type="radio"
                        ...
                        className="hidden"
                      />
                      <div className={`flex-shrink-0 flex items-center justify-center h-9 w-9 rounded-full border-2 ${paymentMethod === "card" ? "bg-blue-600 border-blue-600" : "bg-white dark:bg-slate-900 border-blue-400"}`}>
                        <CreditCard className={`h-5 w-5 ${paymentMethod === "card" ? "text-white" : "text-blue-600"}`} />
                      </div>
                  ...
                  >
                    <input
                      ...
                      className="hidden"
                    />
                    <div className={`flex-shrink-0 flex items-center justify-center h-9 w-9 rounded-full border-2 ${paymentMethod === "bank_transfer" ? "bg-blue-600 border-blue-600" : "bg-white dark:bg-slate-900 border-blue-400"}`}>
                      <Banknote className={`h-5 w-5 ${paymentMethod === "bank_transfer" ? "text-white" : "text-blue-600"}`} />
                    </div>
                    <div className="flex-1">
                      <p className="font-semibold text-slate-900 dark:text-white">
                  ...
                  {paymentMethod === "bank_transfer" && (
                    <div className="rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50/80 dark:bg-blue-950/40 p-4 text-sm space-y-2">
                      ...
                      <div className="flex justify-between gap-3">
                        ...
                        <span className="text-blue-700 dark:text-blue-400 font-bold select-all text-right">{BANK_TRANSFER_IBAN}</span>
                      </div>
                      <div className="flex justify-between gap-3">
                  ...
                  {!isRambursDisabled && (
                    ...
                    >
                      <input
                        ...
                        className="hidden"
                      />
                      <div className={`flex-shrink-0 flex items-center justify-center h-9 w-9 rounded-full border-2 ${paymentMethod === "cash_on_delivery" ? "bg-blue-600 border-blue-600" : "bg-white dark:bg-slate-900 border-blue-400"}`}>
                        <Truck className={`h-5 w-5 ${paymentMethod === "cash_on_delivery" ? "text-white" : "text-blue-600"}`} />
                      </div>
                      <div className="flex-1">
                        <p className="font-semibold text-slate-900 dark:text-white">
                          Ramburs la curier
                  ...
                  {isCardDisabled && (
                    <p className="text-xs text-amber-700 dark:text-amber-400 flex items-start gap-1">
                      ...
                      <span>
                        Plata cu cardul este disponibilă de la{" "}
                        <span className="font-semibold">{MIN_CARD_PAYMENT} lei</span>.
                      </span>
                    </p>
                  )}
                  {isRambursDisabled && totalWithShipping > MAX_RAMBURS_LIMIT && (
                    <p className="text-xs text-amber-700 dark:text-amber-400 flex items-start gap-1">
                      <AlertCircle className="h-3.5 w-3.5 mt-0.5 shrink-0" />
                      <span>
                        Pentru comenzi peste{" "}
                        <span className="font-semibold">{MAX_RAMBURS_LIMIT} lei</span>,
                        plata se face doar cu cardul sau prin ordin de plată.
                      </span>
                    </p>
                  )}
                </div>
                ...
                <div className="border-t border-slate-200 dark:border-slate-800 pt-3 mt-3 space-y-2 text-sm">
                  ...
                  {!hasFreeShipping && (
                    <p className="text-xs text-slate-700 dark:text-slate-300 flex items-start gap-1 mt-1">
                      <AlertCircle className="h-3.5 w-3.5 mt-0.5 text-blue-500" />
                      <span>
                        Mai adaugă{" "}
                        <span className="font-semibold text-blue-600 dark:text-blue-400">
  ...
  if (placing) {
    return (
      <button
        type="button"
        disabled
        className="inline-flex w-full items-center justify-center rounded-xl border border-transparent bg-blue-500 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/30 transition"
      >
        <svg
