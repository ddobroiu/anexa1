...
function getBranding(source?: string | null) {
    ...
    if (s.includes('anexa1.ro')) return { name: 'Anexa1', home: 'https://www.anexa1.ro' };
    if (s.includes('tablou.net')) return { name: 'Tablou.net', home: 'https://www.tablou.net' };
    if (s.includes('visionboard.ro')) return { name: 'VisionBoard', home: 'https://www.visionboard.ro' };
    return { name: 'Anexa1', home: '/' };
}
            ...
            <div className="max-w-2xl w-full">
                <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-8 sm:p-12 shadow-2xl shadow-blue-500/5 text-center relative overflow-hidden">
                    ...
                    <p className="text-slate-600 dark:text-slate-400 mb-10 max-w-md mx-auto font-medium">
                        Îți mulțumim pentru plată. Comanda ta a intrat în rândul pentru producție {branding.name !== 'Anexa1' ? `la ${branding.name} ` : ''}și va fi expediată în cel mai scurt timp.
                    </p>
                    ...
                    <div className="mt-12 pt-8 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
                        ...
                        <div className="flex gap-4">
                            <span className="text-blue-500">Securizat Stripe</span>
