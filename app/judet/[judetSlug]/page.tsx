...
export async function generateMetadata({ params }: Params) {
    ...
    const title = `Panouri și plăci publicitare în județul ${judet.name}${from ? ` – panouri de la ${from.text}` : ""}`;
    const description = `Panouri PVC Forex, alucobond, plexiglas și plăci pentru firme, cu livrare în toate cele ${judet.localitati.length} localități din județul ${judet.name}. Preț calculat pe loc, plată la livrare.`;
...
export default async function JudetPage({ params }: Params) {
    ...
    const waMessage = `Bună ziua! Aș dori o ofertă pentru panouri și plăci publicitare cu livrare în județul ${judet.name}.`;
            ...
            <section className="border-b border-slate-100 bg-gradient-to-b from-blue-50/60 to-white">
                <div className="container mx-auto px-4 py-8 sm:px-6 lg:py-14">
                    ...
                    <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 sm:text-5xl">
                        Panouri și plăci publicitare în județul <span className="text-blue-600">{judet.name}</span>
                    </h1>
                    <p className="mt-4 max-w-2xl text-base text-slate-600 sm:text-lg">
                        Panouri PVC Forex, alucobond, plexiglas și plăci pentru firme, livrate la adresă oriunde în județ.
                        {profile && ` Zonă cunoscută pentru ${profile.industrii.slice(0, 2).join(" și ")}; ${profile.reper} e unul dintre reperele ei.`}
                    </p>
                    <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                        <a href="#localitati" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-7 py-4 text-base font-bold text-white shadow-lg shadow-blue-600/25 transition hover:bg-blue-500">
                        ...
                        <div className="mt-6 flex flex-wrap gap-2">
                            {mainTowns.map((l) => (
                                <Link key={l.slug} href={`/judet/${judet.slug}/${l.slug}`}
                                    className="inline-flex items-center gap-1.5 rounded-xl border border-blue-200 bg-white px-4 py-2.5 font-semibold text-slate-900 hover:border-blue-400 hover:text-blue-700">
                                    <MapPin size={15} className="text-blue-600" /> {l.name}
                                </Link>
                            ))}
                        </div>
                    )}
                        ...
                        <div className="space-y-4 border-t border-slate-100 p-5">
                            {[...byLetter.entries()].map(([letter, list]) => (
                                <div key={letter} className="flex gap-4">
                                    <span className="w-6 shrink-0 font-bold text-blue-600">{letter}</span>
                                    <p className="text-sm leading-7 text-slate-600">
                                        {list.map((l, i) => (
                                            <React.Fragment key={l.slug}>
                                                {i > 0 && <span className="text-slate-300"> · </span>}
                                                <Link href={`/judet/${judet.slug}/${l.slug}`} className="hover:text-blue-700">{l.name}</Link>
                                            </React.Fragment>
                                        ))}
                                    </p>
                                </div>
