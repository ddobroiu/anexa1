...
export async function generateMetadata({ params }: { params: Promise<{ judetSlug: string }> }) {
    ...
    return {
        ...
        alternates: {
            canonical: `https://www.anexa1.ro/judet/${judetSlug}`
        }
    };
}
                    ...
                    __html: JSON.stringify([
                        {
                            ...
                            "@type": "BreadcrumbList",
                            "itemListElement": [
                                { "@type": "ListItem", "position": 1, "name": "Acasă", "item": "https://www.anexa1.ro/" },
                                { "@type": "ListItem", "position": 2, "name": "Județe", "item": "https://www.anexa1.ro/judet" },
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
                        <div className="min-w-0">
                            <h1 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter">
                                {variant.h1Prefix} <span className="text-blue-500">{judet.name}</span>
                            </h1>
                ...
                <div className="mb-20">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {configurators.map((p, i) => (
                            ...
                            >
                                <h3 className="text-lg font-bold text-slate-900 mb-2 group-hover:text-blue-600 transition-colors">{p.title}</h3>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-y-4 gap-x-8">
                        {targetLocalities.map(({ loc }) => (
                            <Link
                                ...
                                className="text-sm font-bold text-slate-500 hover:text-blue-600 transition-colors py-1 flex items-center justify-between group"
                            >
                                {loc.name}
                                <span className="opacity-0 group-hover:opacity-100 text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full transition-all">VEZI</span>
                            </Link>
                        ))}
                    </div>
                </div>
