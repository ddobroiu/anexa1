...
export async function generateMetadata({ params }: { params: Promise<{ judetSlug: string, localitateSlug: string, productSlug: string[] }> }) {
    ...
    const routeUrl = `https://Anexa1.ro/judet/${judet.slug}/${loc.slug}/${productSlug.join('/')}`;
...
export default async function ProductLocalityPage({ params }: { params: Promise<{ judetSlug: string, localitateSlug: string, productSlug: string[] }> }) {
    ...
    // Cross-links to sibling curated localities in the same județ, for the same product.
    const siblingLocalities = getSiblingLocalitySlugs(judet.slug, loc.slug, 4)
        ...
        .filter((siblingLoc): siblingLoc is NonNullable<typeof siblingLoc> => Boolean(siblingLoc));
    ...
    return (
        <div className="bg-[#fafafc] min-h-screen font-sans overflow-x-hidden w-full max-w-full box-border">
                    ...
                    __html: JSON.stringify([
                        {
                            ...
                            "brand": { "@type": "Brand", "name": "Anexa1" },
                            "manufacturer": { "@type": "Organization", "name": "Anexa1", "url": "https://www.Anexa1.ro" },
                        }
                    ])
                }}
            />
            ...
            <div className="w-full bg-white border-b border-slate-100 overflow-hidden">
                <div className="max-w-7xl mx-auto px-4 py-8 md:py-16">
                    ...
                    <nav className="flex items-center gap-2 text-[10px] md:text-xs font-bold text-slate-400 mb-8 uppercase tracking-[0.2em]">
                        <Link href="/judet" className="hover:text-blue-500 transition-colors">Județe</Link> 
                    ...
                    <div className="flex flex-col lg:flex-row gap-8 lg:gap-20 items-start">
                        ...
                        <div className="flex-1 w-full order-2 lg:order-1 lg:pt-8">
                            <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-50 rounded-full text-blue-600 font-black text-[10px] uppercase tracking-widest mb-6 border border-blue-100 italic">
                            ...
                            <h1 className="text-3xl min-[390px]:text-4xl md:text-7xl font-black text-slate-900 leading-[1.1] tracking-tighter mb-6 uppercase italic break-words">
                                {productTitle} <br />
                                <span className="text-blue-500 not-italic uppercase tracking-tighter shadow-sm whitespace-normal">în {loc.name}</span>
                            </h1>
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 mb-12">
                                <Link 
                                    href={shopUrl} 
                                    className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-black text-lg hover:bg-blue-500 transition-all flex items-center justify-center gap-3 shadow-2xl shadow-blue-500/20 active:scale-95 group uppercase italic tracking-tighter"
                                >
                                ...
                                <a 
                                    ...
                                    className="px-10 py-5 bg-white text-slate-900 border-2 border-slate-200 rounded-2xl font-black text-lg hover:border-blue-500 hover:text-blue-600 transition-all flex items-center justify-center gap-3 shadow-sm active:scale-95 group uppercase italic tracking-tighter"
                                >
                                    <MessageCircle size={22} className="group-hover:text-blue-500" />
                                    WHATSAPP
                                </a>
                            </div>
                        ...
                        <div className="flex-1 w-full order-1 lg:order-2 mb-8 lg:mb-0">
                            <div className="relative group">
                                <div className="absolute -inset-4 bg-gradient-to-tr from-blue-500/20 to-transparent rounded-[3rem] blur-3xl opacity-50 group-hover:opacity-100 transition-opacity" />
                                <div className="relative aspect-square bg-slate-50 rounded-[2.5rem] border border-slate-100 flex items-center justify-center p-8 overflow-hidden">
                                    ...
                                    <div className="absolute bottom-6 left-6 right-6 p-4 bg-white/80 backdrop-blur-md rounded-2xl border border-white/50 shadow-lg flex items-center justify-between">
                                        ...
                                        <span className="text-[10px] font-bold text-blue-600">24H Livrare</span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div className="bg-slate-950 py-32 relative overflow-hidden">
                ...
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1/2 h-64 bg-blue-500/10 blur-[120px] rounded-full" />
                
                <div className="max-w-7xl mx-auto px-4 relative z-10">
                    <div className="text-center mb-20">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-blue-500/10 rounded-full text-blue-400 font-black text-[10px] uppercase tracking-[0.2em] mb-6 border border-blue-500/20">
                            <ShieldCheck size={14} /> Certificare Calitate Anexa1
                        </div>
                        <h2 className="text-4xl md:text-7xl font-black text-white mb-6 tracking-tighter uppercase leading-[0.9] max-w-4xl mx-auto">
                            Standardul Premium în <span className="text-blue-400">{loc.name}</span>
                        </h2>
                        <p className="text-slate-400 max-w-2xl mx-auto font-medium text-lg leading-relaxed">
                            Nu facem compromisuri. Fiecare comandă de {product.title.toLowerCase()} expediată în județul {judet.name} trece printr-un control riguros sub semnătura Anexa1.
                        </p>
                    </div>
    
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {[
                            { title: "Culori Vibrante", desc: "Folosim echipamente de ultimă generație cu cerneluri rezistente la raze UV și intemperii, garantate pentru ani de zile.", icon: <Sparkles size={28} />, color: "blue" },
                            ...
                            <div key={i} className="group relative">
                                <div className="absolute inset-0 bg-blue-500/5 blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
                                <div className="relative h-full bg-white/5 backdrop-blur-sm p-10 rounded-[2.5rem] border border-white/10 hover:border-blue-500/30 transition-all duration-500 flex flex-col">
                                    <div className={`w-16 h-16 rounded-2xl bg-slate-900 border border-white/10 flex items-center justify-center mb-8 text-blue-400 group-hover:scale-110 group-hover:bg-blue-500 group-hover:text-white transition-all duration-500`}>
                                        {f.icon}
                                    </div>
                                    <h3 className="text-2xl font-black text-white mb-4 uppercase tracking-tight group-hover:text-blue-400 transition-colors">{f.title}</h3>
                                    ...
                                    <div className="mt-6 pt-6 border-t border-white/5 flex items-center justify-between opacity-50 group-hover:opacity-100 transition-opacity">
                                        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-[0.2em]">Verificat Anexa1</span>
                                        <CheckCircle2 size={14} className="text-blue-500/40 group-hover:text-blue-400 transition-colors" />
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            <div className="max-w-7xl mx-auto px-4 py-24">
                <div className="text-center mb-16">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-50 border border-slate-100 text-slate-500 text-[10px] font-black uppercase tracking-widest mb-4">
                        <Zap size={14} className="text-blue-500" /> TOATE CONFIGURATOARELE
                    </div>
                    ...
                    {CONFIGURATORS_REGISTRY.slice(0, 18).map((config) => {
                        ...
                        return (
                            <Link
                                ...
                                className="group relative flex flex-col items-center text-center rounded-[2rem] border border-slate-200/60 transition-all duration-500 overflow-hidden hover:border-blue-400 hover:shadow-[0_20px_40px_-15px_rgba(16,185,129,0.15)] hover:-translate-y-2 bg-white h-full"
                            >
                                ...
                                <div className="p-4 w-full flex-1 flex flex-col items-center justify-center bg-white relative">
                                    <h4 className="font-black text-[10px] md:text-xs leading-tight tracking-tight transition-all duration-300 text-slate-800 group-hover:text-blue-600 uppercase italic tracking-tighter">
                                        {config.name}
                                    </h4>
                                </div>
                            </Link>
            ...
            <div className="bg-slate-900 py-24">
                <div className="max-w-7xl mx-auto px-4">
                    <div className="flex flex-col md:flex-row items-end justify-between mb-16 gap-6">
                        <div>
                            <p className="text-blue-400 font-bold uppercase tracking-[0.2em] text-[10px] mb-3">Rețeaua Națională</p>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                        {judet.localitati.filter(l => l.slug !== loc.slug).slice(0, 18).map((l, i) => (
                            <Link
                                ...
                                className="px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white/60 hover:text-white hover:bg-white/10 hover:border-blue-500/50 transition-all text-xs font-bold truncate text-center"
                            >
            ...
            {/* Sibling Curated Localities - internal cross-links within the județ */}
            {siblingLocalities.length > 0 && (
                <div className="bg-white py-24 border-t border-slate-100">
                    <div className="max-w-7xl mx-auto px-4">
                        <div className="text-center mb-16">
                            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-50 border border-slate-100 text-slate-500 text-[10px] font-black uppercase tracking-widest mb-4">
                                <MapPin size={14} className="text-blue-500" /> ACOPERIRE JUDEȚEANĂ
                            </div>
                        ...
                        <div className="flex flex-wrap justify-center gap-3">
                            {siblingLocalities.map((siblingLoc) => (
                                <Link
                                    key={siblingLoc.slug}
                                    href={`/judet/${judet.slug}/${siblingLoc.slug}/${productSlug.join('/')}`}
                                    className="px-6 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-600 hover:text-blue-600 hover:bg-white hover:border-blue-400 transition-all text-sm font-bold"
                                >
                                    {productTitle} {siblingLoc.name}
                                </Link>
                            ))}
                        </div>
                    </div>
            ...
            <div className="bg-white py-24 border-t border-slate-100">
                <div className="max-w-4xl mx-auto px-4">
                    <div className="text-center mb-16">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-50 border border-slate-100 text-slate-500 text-[10px] font-black uppercase tracking-widest mb-4">
                            <HelpCircle size={14} className="text-blue-500" /> SUPORT LOCAL {loc.name.toUpperCase()}
                        </div>
