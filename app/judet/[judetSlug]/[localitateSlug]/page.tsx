...
export async function generateMetadata({ params }: { params: Promise<{ judetSlug: string, localitateSlug: string }> }) {
    ...
    const routeUrl = `https://Anexa1.ro/judet/${judet.slug}/${loc.slug}`;
        ...
        <div className="min-h-screen bg-white">
                    ...
                    __html: JSON.stringify([
                        {
                            ...
                            "@type": "Service",
                            "name": `Print și Publicitate Anexa1 ${loc.name}`,
                            "provider": {
                                "@type": "LocalBusiness",
                                "name": "Anexa1",
                                "url": `https://Anexa1.ro/judet/${judet.slug}/${loc.slug}`,
                        ...
                        },
                        {
                            ...
                            "@type": "BreadcrumbList",
                            "itemListElement": [
                                { "@type": "ListItem", "position": 1, "name": "Acasă", "item": "https://Anexa1.ro/" },
                                { "@type": "ListItem", "position": 2, "name": judet.name, "item": `https://Anexa1.ro/judet/${judet.slug}` },
            ...
            <div className="pt-24 pb-12 border-b border-slate-100">
                <div className="container mx-auto px-6">
                    <nav className="text-[10px] font-black text-slate-400 mb-6 flex gap-3 items-center uppercase tracking-widest">
                        ...
                        <span>/</span>
                        <Link href={`/judet/${judet.slug}`} className="hover:text-blue-600 transition-colors">{judet.name}</Link> 
                        <span>/</span>
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
                        <div className="min-w-0">
                            <h1 className="text-4xl md:text-6xl font-black text-slate-900 mb-4 tracking-tighter">
                                {variant.h1Prefix} <span className="text-blue-500">{loc.name}</span>
                            </h1>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 shrink-0">
                            <a
                                ...
                                className="inline-flex items-center justify-center px-6 py-3 rounded-2xl font-black text-sm uppercase tracking-widest border-2 border-slate-200 bg-white text-slate-900 hover:border-blue-500 hover:text-blue-600 transition-all shadow-sm active:scale-[0.99]"
                            >
                                WhatsApp
                            </a>
                            <Link
                                href="/contact"
                                className="inline-flex items-center justify-center px-6 py-3 rounded-2xl font-black text-sm uppercase tracking-widest bg-blue-600 text-white hover:bg-blue-500 transition-all shadow-xl shadow-blue-600/20 active:scale-[0.99]"
                            >
                                Cere ofertă
                            </Link>
                        </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
                        {configurators.map((p: any) => {
                            ...
                            return (
                                ...
                                >
                                    <div className="w-full aspect-square bg-slate-50 rounded-[4rem] overflow-hidden border border-slate-100 group-hover:border-blue-500 group-hover:shadow-2xl group-hover:-translate-y-4 transition-all duration-700 relative">
                                        <Image
                                            src={p.image || '/placeholder.png'}
                                            alt={`${p.name} personalizat în ${loc.name}, județul ${judet.name} - Anexa1`}
                                            fill
                                    ...
                                    <div>
                                        <h3 className="text-2xl font-black text-slate-900 group-hover:text-blue-600 transition-colors mb-2">{p.name}</h3>
            ...
            <section className="py-40 bg-slate-900 text-white relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1200px] h-[1200px] bg-blue-500/5 rounded-full blur-[200px]"></div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-16 text-left">
                        {variant.trustFacts.map((item, i) => (
                            <div key={i} className="space-y-6">
                                <div className="text-blue-500 font-black text-4xl">0{i+1}.</div>
