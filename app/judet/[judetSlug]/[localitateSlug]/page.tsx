...
const TOP = ["pvc-forex", "alucobond", "plexiglass", "polipropilena", "carton", "autocolante", "banner", "afise"];
...
export async function generateMetadata({ params }: Params) {
    ...
    const from = getFromPrice(["pvc-forex"]);
    const title = `Panouri și plăci publicitare în ${loc.name}${from ? ` – panouri de la ${from.text}` : ""}`;
    const description = `Panouri PVC Forex, alucobond, plexiglas și plăci pentru firme, cu livrare în ${loc.name}, jud. ${judet.name}. Preț calculat pe loc, producție în 1-3 zile, plată la livrare.`;
    const routeUrl = `${siteConfig.url}/judet/${judet.slug}/${loc.slug}`;

    return {
        title,
        description,
        openGraph: { title, description, url: routeUrl, siteName: "Anexa1", locale: "ro_RO", type: "website" },
...
export default async function LocalitatePage({ params }: Params) {
    ...
    const generated = buildLocalContent({
        brand: "anexa1",
        productTitle: "panouri și plăci publicitare",
        productSlug: "print",
        locName: loc.name,
        locSlug: loc.slug,
    ...
    const intro = `Panouri PVC Forex, alucobond, plexiglas și plăci pentru firme, livrate la adresa ta din ${loc.name}. ${localFact}`.trim();
    ...
    const bannerFrom = getFromPrice(["pvc-forex"]);
    const pageUrl = `${siteConfig.url}/judet/${judet.slug}/${loc.slug}`;
    const waMessage = `Bună ziua! Aș dori o ofertă pentru panouri și plăci publicitare cu livrare în ${loc.name}, jud. ${judet.name}.`;
    ...
    return (
        <div className="bg-white">
                    ...
                    __html: JSON.stringify([
                        {
                            ...
                            "@type": "Service",
                            name: `Panouri și plăci publicitare în ${loc.name}`,
                            serviceType: "Panouri și plăci publicitare",
                            areaServed: { "@type": "City", name: loc.name, containedInPlace: { "@type": "AdministrativeArea", name: `Județul ${judet.name}` } },
                            provider: { "@type": "Organization", name: "Anexa1", url: siteConfig.url, telephone: siteConfig.phone },
                            url: pageUrl,
                        },
                        {
                            "@context": "https://schema.org",
                ...
                <div className="container mx-auto grid gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[1.4fr_1fr] lg:items-center lg:py-14">
                    <div>
                        <nav aria-label="Breadcrumb" className="mb-4 flex flex-wrap items-center gap-1.5 text-xs text-slate-500">
                            <Link href="/judet" className="hover:text-blue-700">Județe</Link>
                            <span aria-hidden>/</span>
                            <Link href={`/judet/${judet.slug}`} className="hover:text-blue-700">{judet.name}</Link>
                            <span aria-hidden>/</span>
                            <span className="text-slate-800">{loc.name}</span>
                        </nav>
                        <p className="mb-3 inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1 text-xs font-medium text-blue-700 ring-1 ring-blue-200">
                            <Truck size={14} /> Livrăm în {loc.name}, jud. {judet.name}
                        </p>
                        <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 sm:text-5xl">
                            Panouri și plăci publicitare în <span className="text-blue-600">{loc.name}</span>
                        </h1>
                        <p className="mt-4 max-w-2xl text-base text-slate-600 sm:text-lg">{intro}</p>

                        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                            <a href="#produse" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-7 py-4 text-base font-bold text-white shadow-lg shadow-blue-600/25 transition hover:bg-blue-500">
                        ...
                        <a href={`tel:${siteConfig.phone.replace(/\s/g, "")}`} className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-slate-600 hover:text-blue-700">
                        ...
                        <ul className="mt-6 flex flex-wrap gap-x-5 gap-y-2 text-sm text-slate-700">
                            {["Preț calculat pe loc", "Producție în 1-3 zile", "Plată la livrare", "Livrare DPD"].map((t) => (
                                <li key={t} className="inline-flex items-center gap-1.5"><Check size={16} className="text-blue-600" /> {t}</li>
                            ))}
                        </ul>
                    </div>
                                    ...
                                    <li key={p.id}>
                                        <Link href={`/judet/${judet.slug}/${loc.slug}/${p.slug || p.id}`} className="group flex items-center gap-3 py-3">
                                            ...
                                            <span className="min-w-0 flex-1">
                                                <span className="block truncate font-semibold text-slate-900 group-hover:text-blue-700">{p.name}</span>
                                                {from && <span className="text-sm text-slate-500">de la <b className="text-slate-900">{from.text}</b></span>}
                                            </span>
                                            <span className="rounded-xl bg-blue-600 px-3 py-2 text-xs font-bold text-white group-hover:bg-blue-500">Comandă</span>
                                        </Link>
                                    </li>
                                );
                            })}
                    ...
                    <ol className="mt-8 grid gap-4 sm:grid-cols-3">
                        {[
                            ...
                            <li key={s.t} className="rounded-2xl border border-slate-200 bg-white p-5">
                                <span className="flex size-10 items-center justify-center rounded-xl bg-blue-600 text-white"><s.icon size={20} /></span>
                                <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-blue-700">Pasul {i + 1}</p>
                        ...
                        {faq.map((f, i) => (
                            <details key={f.q} className="group p-5" open={i === 0}>
                                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-semibold text-slate-900">
                                    {f.q}
                                    <span className="text-blue-600 transition group-open:rotate-45" aria-hidden>+</span>
                                </summary>
                        ...
                        <div className="mt-5 flex flex-wrap gap-2">
                            {neighbours.map((l) => (
                                <Link key={l.slug} href={`/judet/${judet.slug}/${l.slug}`}
                                    className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm text-slate-700 hover:border-blue-300 hover:text-blue-700">
                                    {l.name}
                                </Link>
                            ))}
                            <Link href={`/judet/${judet.slug}`} className="rounded-full px-4 py-2 text-sm font-semibold text-blue-700 hover:underline">
            ...
            <WhatsAppBar message={waMessage} price={bannerFrom?.text} label="panouri de la" />
        </div>
    );
}
