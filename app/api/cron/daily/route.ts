...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log('ðŸ§¹ [Cleanup] Deactivating expired codes...');
    stats.expiredCodes = await deactivateExpiredCodes();

    // 2. ABANDONED CART RECOVERY: Process all abandoned carts
    console.log('ðŸ“§ [Email] Processing abandoned cart recovery...');
    ...
    console.log(`ðŸ“Š Found ${abandonedCarts.length} abandoned carts to process`);
    ...
    for (const cart of abandonedCarts) {
      try {
        ...
        if (success) {
          ...
          stats.sent++;
          console.log(`âœ… Sent ${emailType} email to ${cart.email} for ${cart.configuratorId}`);
        } else {
    ...
    // console.log('ðŸŽ [Email] Processing post-purchase follow-ups...');
    ...
    //     }
    //   } catch (error) {
    //     console.error(`âŒ Error follow-up order ${order.orderNo}:`, error);
    //   }
    // }
    ...
    console.log(`
ðŸŽ¯ [Daily Cron] Completed in ${runtime}s
ðŸ“Š Stats:
   â€¢ Abandoned carts found: ${stats.newAbandoned}
   â€¢ Emails processed: ${stats.processed}  
   â€¢ Emails sent: ${stats.sent}
   â€¢ Follow-ups sent: ${stats.followUpsSent}
   â€¢ Expired codes cleaned: ${stats.expiredCodes}
   â€¢ Errors: ${stats.errors.length}
      ...
      try {
        ...
        if (resend) {
          await resend.emails.send({
            from: 'Anexa1 System <system@Anexa1.ro>',
            to: process.env.ADMIN_EMAIL,
            subject: `📊 Raport zilnic marketing Anexa1.ro - ${stats.sent} emailuri trimise`,
            html: `
            ...
            <p><small>Sistem automat Anexa1.ro</small></p>
            `
          });
        }
      } catch (emailError) {
    ...
    console.error('âŒ [Daily Cron] Fatal error:', error);
