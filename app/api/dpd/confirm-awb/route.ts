  ...
  try {
    ...
    if (!shipment?.recipient || !shipment?.service || !shipment?.content || !shipment?.payment) {
      return NextResponse.json(
        { ok: false, message: 'Parametri lipsÄƒ: recipient, service, content, payment' },
        { status: 400 }
      );
    }
    ...
    if (created?.error || !created?.id) {
      return NextResponse.json(
        { ok: false, message: created?.error?.message || 'Eroare creare expediÈ›ie', error: created?.error },
        { status: 400 }
      );
    }
  ...
  const trackingUrl = trackingUrlForAwb(shipmentId);
    ...
    if (email) {
      try {
        ...
        if (!resend) throw new Error('RESEND_API_KEY lipsÄƒ');
        const subject = `AWB DPD ${shipmentId}`;
        const html = `<p>BunÄƒ${name ? ' ' + name : ''},</p>
<p>ExpediÈ›ia ta a fost confirmatÄƒ. AWB: <strong>${shipmentId}</strong>.</p>
<p>PoÈ›i urmÄƒri statusul livrÄƒrii aici: <a href="${trackingUrl}">${trackingUrl}</a> (introdu AWB-ul dacÄƒ este necesar).</p>
<p>GÄƒseÈ™ti ataÈ™at fiÈ™ierul PDF al etichetei (A6) pentru livrare.</p>
<p>ÃŽÈ›i mulÈ›umim!</p>`;
        const attachments = base64
          ? [{ filename: `DPD_${shipmentId}.pdf`, content: Buffer.from(base64, 'base64') }]
          : undefined;
        await resend.emails.send({
          from: process.env.EMAIL_FROM || 'contact@Anexa1.ro',
          to: email,
    ...
    return NextResponse.json({ ok: false, message: 'Eroare internÄƒ' }, { status: 500 });
  }
}
