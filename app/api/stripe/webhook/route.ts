...
export async function POST(req: NextRequest) {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        // FILTRARE DUPĂ SURSĂ
        const allowedSources = ['anexa1.ro', 'visionboard.ro', 'prynt.ro', 'euprint.ro', 'tablou.net', 'shopprint.ro'];
        const source = String(session.metadata?.source || 'anexa1.ro').toLowerCase();

        if (!allowedSources.includes(source)) {
            console.log(`[Anexa1 Webhook] Ignored event for source: ${source}`);
            return NextResponse.json({ received: true, ignored: true });
        }

        try {
            console.log(`[Anexa1 Webhook] Processing session: ${session.id}`);
            ...
            if (existingOrder) {
                console.log(`[Anexa1 Webhook] Order already exists for session ${session.id}. Skipping.`);
            ...
            if (!pendingCheckout) {
                console.warn(`[Anexa1 Webhook] No pending checkout found for session: ${session.id}`);
                console.warn('[Anexa1 Webhook] Falling back to metadata:', JSON.stringify(session.metadata, null, 2));
                ...
                if (session.metadata?.address_email) {
                    ...
                    console.log('[Anexa1 Webhook] Constructed fallback checkoutData from metadata');
                } else {
                    console.error('[Anexa1 Webhook] CRITICAL: No pending checkout AND no address_email in metadata.');
            ...
            console.log(`[Anexa1 Webhook] Order fulfilled successfully: ${result.orderNo}`);

        } catch (error) {
            console.error('[Anexa1 Webhook] Error fulfilling order:', error);
