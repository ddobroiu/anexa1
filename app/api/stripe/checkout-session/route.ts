...
export async function POST(req: NextRequest) {
  ...
  const userId = sessionUser?.user ? (sessionUser.user as any).id : null;
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      payment_intent_data: { metadata: { group: 'print', project: 'anexa1', ...(mdVid && { md_vid: mdVid }), source: 'anexa1.ro' } },
    ...
    await prisma.pendingCheckout.create({
      data: {
        ...
        checkoutData: { cart, address, billing, marketing, userId },
