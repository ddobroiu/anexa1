...
const VERIFY_TOKEN = process.env.META_VERIFY_TOKEN || "whatsapp_Anexa1_123";

// Cloudinary and OpenAI will be initialized inside handlers

// const openai = new OpenAI({...}) moved inside POST
...
// FIX: Memory leak prevention - TTL È™i limitare
...
// Cleanup function pentru conversaÈ›ii vechi
function cleanupOldConversations() {
  ...
  // Limitare numÄƒr maxim conversaÈ›ii
...
async function processWhatsAppImage(imageId: string, fromNumber: string) {
  try {
    ...
    if (!token) {
      console.error("âŒ META_API_TOKEN lipseÈ™te din configurare");
      ...
      if (messages && messages[0]) {
        ...
        if (msgType === "image") {
          ...
          if (uploadResult) {
            ...
            const sendResult = await sendYesNoQuestion(from, "Am primit imaginea ta! ðŸ“¸ Am salvat-o Ã®n contul tÄƒu. DoreÈ™ti o ofertÄƒ pentru ea?");
        ...
        // Cleanup conversaÈ›ii vechi periodic
        if (Math.random() < 0.1) cleanupOldConversations(); // 10% È™ansÄƒ la fiecare mesaj
...
ðŸ”´ IMPORTANT WhatsApp:
- MAX 2 PROPOZIÈšII per rÄƒspuns!
- FoloseÈ™te OBLIGATORIU ||OPTIONS: [...]|| pentru orice alegere
- WhatsApp LIMITÄ‚: Max 3 butoane (alege cele mai importante)
- Exemplu corect: "Ce material? ||OPTIONS: [\"Frontlit 440g\", \"Frontlit 510g\", \"Verso\"]||"
- NU scrie opÈ›iunile Ã®n text, doar Ã®n ||OPTIONS||!`;
        ...
        const needsRAG = /(banner|afiÈ™|autocolant|canvas|tapet|rollup|window|pliant|flayer|fonduri|plexiglas|forex|alucobond|carton|material|dimensiune|preÈ›)/i.test(textBody.toLowerCase());
        ...
        if (finalReply && finalReply.trim().length > 0) {
          ...
          // Caz 1: Cerere explicitÄƒ de JudeÈ›
          ...
          // Caz 1.6: Tag-ul ||BUTTON:text:url|| pentru download link
            ...
            if (buttonMatch) {
              ...
              sendResult = await sendWhatsAppMessage(from, `${cleanText}\n\nðŸ”— ${buttonText}: ${buttonUrl}`);
            } else {
              sendResult = await sendWhatsAppMessage(from, replyText);
            }
          }
          // Caz 2: ÃŽntrebÄƒri de tip "Da/Nu"
          else if (
            lowerReply.includes("?") &&
          ...
          // Caz 3: Meniu Principal sau OpÈ›iuni detectate
          ...
          // LOGARE ÃŽN ADMIN DOAR DACÄ‚ S-A TRIMIS SAU LOGARE EROARE
          if (sendResult) {
            // Mesajul s-a trimis
            const msgsToLog = [
              { role: 'user', content: textBody },
            ...
            // EROARE LA TRIMITERE
            console.error(`âŒ FAILED to send message to ${from}`);
            await logConversation('whatsapp', from, [
              { role: 'user', content: textBody },
              { role: 'assistant', content: `âš ï¸ EROARE SISTEM: Mesajul nu a ajuns la client. VerificÄƒ logurile serverului.\nConÈ›inut intenÈ›ionat: ${finalReply}` }
            ], userId);
          }
        }
