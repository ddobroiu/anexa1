...
export async function generateMetadata({ params }: { params: Promise<{ industrySlug: string }> }): Promise<Metadata> {
    ...
    return {
        ...
        alternates: { canonical: `https://Anexa1.ro/industrii/${ind.slug}` }
    };
}
    ...
    const faqSchema = {
        ...
        "mainEntity": [
            ...
            },
            {
                ...
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": `Da, Anexa1 oferă soluții personalizate și pachete de preț avantajoase pentru companii din industria ${ind.name.split(' (')[0]} care au nevoi recurente sau volume mari de print.`
                }
            }
        ]
    };
        ...
        <div className="min-h-screen bg-white">
            ...
            <div className="relative pt-24 min-h-[70vh] flex items-center overflow-hidden">
                <div className="container mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
                    <div>
                        <Link href="/industrii" className="inline-flex items-center gap-2 text-xs font-black text-blue-600 uppercase tracking-widest mb-8 hover:gap-3 transition-all">
                            &larr; Toate Industriile
                        </Link>
                        <h1 className="text-5xl md:text-[5.5rem] font-black text-slate-900 tracking-tighter leading-[0.8] mb-8">
                            {ind.name.split(' (')[0]} <br />
                            <span className="text-blue-500 italic">Soluții Expert.</span>
                        </h1>
                        <p className="text-xl text-slate-600 font-medium leading-relaxed mb-10 max-w-xl">
                            {ind.longDescription}
                        </p>
                        <div className="flex flex-wrap gap-4">
                            <Link href="#produse" className="bg-slate-900 text-white px-10 py-5 rounded-2xl font-black text-lg hover:bg-blue-600 transition-all uppercase tracking-widest shadow-xl">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {ind.benefits.map((benefit, i) => (
                            <div key={i} className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
                                <div className="w-12 h-12 bg-blue-100 text-blue-600 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-blue-500 group-hover:text-white transition-all">
                ...
                <div className="container mx-auto px-6">
                    <div className="text-center mb-20">
                        <h2 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter uppercase italic leading-[0.8]">
                            Toate materialele <br /> <span className="text-blue-500">pentru {ind.slug}.</span>
                        </h2>
                        ...
                        {allProducts.map((p) => {
                            ...
                            return (
                                <Link href={productUrl} key={p.id} className="group flex flex-col">
                                    <div className={`aspect-square relative rounded-[2.5rem] overflow-hidden border transition-all duration-500 group-hover:-translate-y-3 group-hover:shadow-2xl ${rec ? 'bg-blue-50 border-blue-200 group-hover:border-blue-400' : 'bg-slate-50 border-slate-100 group-hover:border-slate-300'}`}>
                                        <Image
                                            src={image}
                                            alt={p.name}
                                            fill
                                    ...
                                    <div className="mt-4 px-1">
                                        <h3 className={`text-sm md:text-base font-black transition-colors leading-tight tracking-tight ${rec ? 'text-slate-900 group-hover:text-blue-600' : 'text-slate-700 group-hover:text-slate-900'}`}>
                                            {p.name}
                                        </h3>
                ...
                <div className="bg-slate-950 rounded-[4rem] p-12 md:p-32 text-center relative overflow-hidden isolate">
                    ...
                    <div className="max-w-3xl mx-auto">
                        <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter mb-8 leading-[0.9]">
                            Vrei un pachet personalizat <br /> <span className="text-blue-400">pentru afacerea ta?</span>
                        </h2>
                        ...
                        <div className="flex flex-col sm:flex-row justify-center gap-4">
                            <Link href="/contact" className="bg-white text-slate-950 px-10 py-5 rounded-2xl font-black text-lg hover:bg-blue-400 transition-all uppercase tracking-widest">
                                Discută cu un Expert
                            </Link>
                            <a href="https://wa.me/40750473111" className="bg-blue-500 text-white px-10 py-5 rounded-2xl font-black text-lg hover:bg-blue-600 transition-all uppercase tracking-widest flex items-center justify-center gap-3 shadow-lg">
                                WhatsApp Instant
                            </a>
                        </div>
                    </div>
