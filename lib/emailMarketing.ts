...
export function generateNewsletterCampaign(theme: 'weekly' | 'promotional' | 'educational') {
  ...
  switch (theme) {
    case 'weekly':
      return {
        subject: 'Săptămâna aceasta la Anexa1: Noutăți și Oferte',
...
export async function sendConfiguratorWelcomeEmail(subscription: NewsletterSubscription) {
  ...
  const html = getHtmlTemplate({
    ...
    footerText: "Mulțumim că te-ai alăturat comunității Anexa1!"
  });
...
export async function sendPostPurchaseFollowUp(email: string, name: string, orderId: string, orderNo: number) {
  ...
  const html = getHtmlTemplate({
    title: `Bună ${name}, cum ți s-au părut produsele Prynt?`,
    ...
    footerText: "Echipa Anexa1 vă mulțumește!"
  });
  ...
  try {
    await sendEmail({
      from: 'Anexa1 <contact@Anexa1.ro>',
      to: email,
      subject: `🎁 Avem un cadou pentru tine (Comanda ta #${orderNo} la Anexa1)`,
      html: enhancedHtml,
    });
    return true;
  } catch (error) {
...
export async function sendAbandonedCartEmail({ email, configuratorId, cartData, emailType, discountPercent = 0 }: {
  ...
  try {
    switch (emailType) {
      case 'gentle':
        subject = `🎨 Te așteaptă ceva special la Prynt!`;
